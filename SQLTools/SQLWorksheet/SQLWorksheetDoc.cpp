...
void CPLSWorksheetDoc::Init ()
{
    ...
    {
        std::shared_ptr<BindVarHolder> m_bindVarHolder;
...
void CPLSWorksheetDoc::ShowSelectResult (std::unique_ptr<OciAutoCursor>& cursor, double lastExecTime)
{
...
void CPLSWorksheetDoc::OnSqlDescribe()
{
    try { EXCEPTION_FRAME;
        ...
        {
            int inx = m_pEditor->PosToInx(sel.end.line, sel.end.column);
            SqlAliasResolution aliasRes;
            if (wbuff.find(L'.') != std::wstring::npos)
                aliasRes = ResolveSqlAlias(m_pEditor, sel.end.line, inx);
            auto& [alias, table, cols, column] = aliasRes;

            if (!table.empty())
            {
                std::string statusMsg = "Alias " + alias + " is resolved to object " + table;
                if (!column.empty()) statusMsg += ", column " + column;
                Global::SetStatusText(statusMsg + ", retrieving its components...", true);

                if (GetSQLToolsSettings().GetAutocompleteUseLowercaseForAliases())
                    for (auto& c : alias) c = (char)::tolower((unsigned char)c);

                viewer.ShowObject(table, alias, column);
            }
            else if (cols.size() > 0)
            {
                Global::SetStatusText("The alias " + alias + " was resolved to subquery. Only tables and views are supported!", true);
                MessageBeep((UINT)-1);
                AfxThrowUserException();
            }
            else
...
void CPLSWorksheetDoc::DoSqlLoad (bool withDbmsMetadata)
{
    try { EXCEPTION_FRAME;
        ...
        if (m_pEditor->GetBlockOrWordUnderCursor(wbuff,  sel, true/*only one line*/, delims.c_str()))
        {
            ...
            if (!table.empty())
            {
                Global::SetStatusText("Alias " + alias + " is resolved to object " + table + ".", true);
            }
            else if (cols.size() > 0)
            {
                Global::SetStatusText("The alias " + alias + " was resolved to subquery. Only tables and views are supported!", true);
            ...
            else
            {
                ...
                table = performer.GetSqlText();
            }

            BkgdRequestQueue::Get().Push(TaskPtr(new BackgroundTask_FindAndLoadDLL(table, withDbmsMetadata)));
        }
...
void CPLSWorksheetDoc::OnSqlHelp()
{
    ...
    if (m_pEditor->GetBlockOrWordUnderCursor(key,  sel, true/*only one line*/))
    {
        ...
        ::HtmlHelp(GetDesktopWindow(), helpPath, HH_KEYWORD_LOOKUP, (DWORD_PTR)&link);
    }
    else
...
void CPLSWorksheetDoc::OnEditAutocomplete ()
{
    try {
        ...
        if (theApp.GetConnectOpen())
        {
            ...
            string::size_type pos = delims.find('.');
            if (pos != string::npos) delims.erase(pos, 1);
            pos = delims.find('*'); // SELECT e.*^ FROM ...
            if (pos != string::npos) delims.erase(pos, 1);

            OpenEditor::Position orgPos = m_pEditor->GetPosition();
            bool foundDot = false, posAdjusted = false, restoreCaret = false;
            std::wstring wbuff;
            OpenEditor::Square sqr;
            // Detect if caret is exactly between '.' and ',' (or ';')
            {
                Common::OEStringW lineBuff;
                if (orgPos.line < m_pEditor->GetLineCount())
                {
                    int inx = m_pEditor->PosToInx(orgPos.line, orgPos.column, true);
                    m_pEditor->GetLineW(orgPos.line, lineBuff);
                    if (inx > 0 && inx < lineBuff.length())
                    {
                        wchar_t prevCh = lineBuff.at(inx - 1);
                        wchar_t curCh  = lineBuff.at(inx);
                        if (prevCh == L'.' 
                        && (curCh == L',' || curCh == L';' || curCh == L'(' || curCh == L')'))
                            restoreCaret = true;
                    }
                }
            }
            ...
            {
                // If we are exactly on a delimiter (e.g., after alias.column), step left and try again
                if (wbuff.size() == 1)
                {
                    wchar_t ch = wbuff[0];
                    bool isDelimChar = (delims.find(static_cast<char>(ch)) != std::string::npos)
                                       || ch == L',' || ch == L';' || ch == L'(' || ch == L')';
                    if (isDelimChar)
                    {
                        auto adj = m_pEditor->GetPosition();
                        if (adj.column > 0)
                        {
                            --adj.column;
                            m_pEditor->MoveTo(adj);
                            posAdjusted = true;
                            // Re-read word under cursor after moving left
                            m_pEditor->GetBlockOrWordUnderCursor(wbuff, sqr, true, delims.c_str());
                        }
                    }
                }

                if (!wbuff.empty() && *wbuff.rbegin() == L'.')
                {
                    foundDot = true;
                }
                else
                {
                    // Within the word under cursor, find the closest '.' before the caret
                    OpenEditor::Position curPos = m_pEditor->GetPosition();
                    int caretInx = m_pEditor->PosToInx(curPos.line, curPos.column, true);
                    int wordStartInx = m_pEditor->PosToInx(sqr.start.line, sqr.start.column, true);

                    int relMax = static_cast<int>(wbuff.size());
                    int relCur = std::max(0, std::min(relMax, caretInx - wordStartInx));

                    int relDot = -1;
                    for (int i = relCur - 1; i >= 0; --i)
                    {
                        if (wbuff[i] == L'.') { relDot = i; break; }
                    }

                    if (relDot >= 0)
                    {
                        int dotAbsInx = wordStartInx + relDot;
                        OpenEditor::Position newPos;
                        newPos.line = curPos.line;
                        newPos.column = m_pEditor->InxToPos(curPos.line, dotAbsInx + 1); // after '.'
                        m_pEditor->MoveTo(newPos);
                        foundDot = true;
                        posAdjusted = true;
                    }
                }
            ...
            if (foundDot)
            {
                ...
                int inx = m_pEditor->PosToInx(sqr.end.line, sqr.end.column);
                auto [alias, table, cols, column] = ResolveSqlAlias(m_pEditor, sqr.end.line, inx);

                auto s = GetSQLToolsSettings();
                AutocompleteTemplateParams params;
                params.sortSubobjects               = s.GetAutocompleteSortSubobjects();
                params.enableMultiColumnSelection   = s.GetAutocompleteEnableMultiColumnSelection();
                params.insertColumnsOnSeparateLines = s.GetAutocompleteInsertColumnsOnSeparateLines();
                params.useLeadingComma              = s.GetAutocompleteUseLeadingComma();
                params.useLowercaseForAliases       = s.GetAutocompleteUseLowercaseForAliases();
                //TODO: convert alias to lowercase right here and drop params.useLowercaseForAliases

                OpenEditor::TemplatePtr tmpl;
                if (!cols.empty())
                {
                    Global::SetStatusText("The column list is resolved for " + alias + ".", true);
                    tmpl.reset(new AutocompleteColumnsTemplate(cols, alias, params));
                }
                else if (!table.empty())
                {
                    Global::SetStatusText("Alias " + alias + " is resolved to object " + table + ", retrieving its components...", true);
                    tmpl = SessionCache::GetAutocompleteSubobjectTemplate(table, alias, params);
                    if (tmpl.get() && tmpl->GetCount() == 0)
                    {
                        Global::SetStatusText("Alias " + alias + " is resolved to object " + table + ", but object is not found!", true);
                        MessageBeep((UINT)-1);
                    }
                }
                else
                {
                    // Try schema autocomplete first; fall back to sub-object lookup
                    tmpl = SessionCache::GetAutocompleteTemplateForSchema(alias);
                    if (tmpl.get() && tmpl->GetCount() > 0)
                    {
                        Global::SetStatusText("Schema " + alias + " resolved, showing objects.", true);
                    }
                    else
                    {
                        Global::SetStatusText("Lookup is called for " + alias + ", retrieving its components...", true);
                        tmpl = SessionCache::GetAutocompleteSubobjectTemplate(alias, alias, params);
                        if (tmpl.get() && tmpl->GetCount() == 0)
                        {
                            Global::SetStatusText("Object " + alias + " is not found!", true);
                            MessageBeep((UINT)-1);
                        }
                    }
                }
...
struct BackgroundTask_DoSqlQuickQuery : ServerBackgroundThread::Task
{
    ...
    std::unique_ptr<OciAutoCursor> m_cursor;
...
void CPLSWorksheetDoc::DoSqlQuickQuery (const char* query, const char* message) // accepts <TABLE> placeholder
{
    ...
    try { EXCEPTION_FRAME;
        ...
        if (m_pEditor->GetBlockOrWordUnderCursor(wbuff, sel, false/*only one line*/, delims.c_str()))
        {
            ...
            SqlAliasResolution aliasRes;
            if (wbuff.find(L'.') != std::wstring::npos)
                aliasRes = ResolveSqlAlias(m_pEditor, sel.end.line, inx, &subquery, false);
            auto& [alias, table, cols, column] = aliasRes;
            ...
            else if (!subquery.empty())
            {
                Global::SetStatusText("The alias " + alias + " was resolved to subquery.", true);
                table = subquery;
            }
            else
            {
...
void CPLSWorksheetDoc::OnContextMenuInit (CMenu* pMenu)
{
    if (pMenu)
    {
        bool canFormat = !m_pEditor->IsSelectionEmpty() 
            && (m_pEditor->GetBlockMode() == OpenEditor::EBlockMode::ebtStream);
        UINT uFormatFlags = canFormat ? MF_STRING | MF_BYPOSITION : MF_STRING | MF_BYPOSITION | MF_DISABLED;
        pMenu->InsertMenu(0, MF_SEPARATOR | MF_BYPOSITION);
        pMenu->InsertMenu(0, uFormatFlags, ID_EDIT_FORMAT_SQL, L"&Format SQL");
        pMenu->InsertMenu(0, MF_SEPARATOR|MF_BYPOSITION);
