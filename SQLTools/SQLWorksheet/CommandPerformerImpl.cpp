...
CommandPerformerImpl::CommandPerformerImpl (DocumentProxy& proxy, OciConnect& connect, std::shared_ptr<BindVarHolder>& bindVarHolder, const CommandPerformerSettings& settings)
    : m_proxy(proxy),
    m_connect(connect),
    m_settings(settings),
...
std::unique_ptr<OciAutoCursor> CommandPerformerImpl::GetCursorOwnership ()
{
    std::unique_ptr<OciAutoCursor> cursor(new OciAutoCursor(m_connect, 1000));
...
void CommandPerformerImpl::DoConnect (CommandParser& commandParser, const string& _connectStr)
{
    ...
    commandParser.SetSubstitution("_USER", m_connect.GetUID());
    commandParser.SetSubstitution("_CONNECT_IDENTIFIER", m_connect.GetConnectIdentifier());
}
...
void CommandPerformerImpl::DoDisconnect (CommandParser& commandParser)
{
    ...
    commandParser.SetSubstitution("_USER", "");
    commandParser.SetSubstitution("_CONNECT_IDENTIFIER", "");
}
...
void CommandPerformerImpl::PrintBind (CommandParser& commandParser, const string& name)
{
    ...
    std::shared_ptr<OCI8::Variable> bindVar = m_bindVarHolder->GetBindVariable(name);
    ...
    if (bindVar.get())
    {
        ...
        {
            m_cursor->Prepare(string("select :" + name + " from sys.dual").c_str());
