...
// somethink like if (match(tokens, (etVARIABLE, etUNKNOWN, etNUMBER))) ...
		...
		if (s_type == "VARCHAR2")
			...
			type = EBVT_NUMBER;
		else if (s_type == "BINARY_FLOAT")
			type = EBVT_BINARY_FLOAT;
		else if (s_type == "BINARY_DOUBLE")
			type = EBVT_BINARY_DOUBLE;
		else if (s_type == "CLOB")
			type = EBVT_CLOB;
		else if (s_type == "DATE")
			type = EBVT_DATE;
		...
		// variable size is required for CHAR and VARCHAR2
		if (type == EBVT_VARCHAR2 || type == EBVT_CHAR)
        {
		...
		if (type == EBVT_CHAR && i_size > 2000U)
			throw CommandParserException("CHAR bind variable length cannot exceed 2000.");
	}
