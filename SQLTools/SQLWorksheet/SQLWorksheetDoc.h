...
#include "BindVariableTypes.h"
    ...
    // Extended alias resolver that returns alias, table and columns
    struct SqlAliasResolution { std::string alias; std::string table; std::vector<std::string> columns; std::string column; };
    SqlAliasResolution ResolveSqlAlias (COEditorView* pEditor, int line, int pos, std::string* outSubquery = nullptr, bool singleTableFromSimpleSubquery = true);
...
public:
    ...
    void ShowSelectResult (std::unique_ptr<OciAutoCursor>& cursor, double lastExecTime);
