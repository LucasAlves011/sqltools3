...
int BindVarHolder::Append(OciConnect& connect, const BindMapKey& name, EBindVariableTypes type, ub2 size)
{
	...
	case EBVT_NUMBER:
		...
		fld = shared_ptr<Variable>(new NumberVar(connect, "TM9"));
		break;
	case EBVT_BINARY_FLOAT:
		fld = shared_ptr<Variable>(new BinaryFloatVar());
		break;
	case EBVT_BINARY_DOUBLE:
		fld = shared_ptr<Variable>(new BinaryDoubleVar());
		break;
	case EBVT_VARCHAR2:
		fld = shared_ptr<Variable>(new StringVar(size));
		break;
	case EBVT_CHAR:
		fld = shared_ptr<Variable>(new CharVar(size));
		break;
	case EBVT_DATE:
		fld = shared_ptr<Variable>(new DateVar(connect, GetSQLToolsSettings().GetGridNlsDateFormat()));
        break;
    case EBVT_CLOB:
		fld = shared_ptr<Variable>(new CLobVar(connect, gridMaxLobLength));
        break;
    case EBVT_REFCURSOR:
		fld = shared_ptr<Variable>(new RefCursorVariable(connect));
        break;
    }
...
std::shared_ptr<OCI8::Variable> BindVarHolder::GetBindVariable (const std::string name)
{
	BindMap :: iterator it = m_boundFields.find(name);
...
void BindVarHolder::GetTypeString (const Variable& var, string& str) const
{
	...
	else if (ti == typeid(BinaryFloatVar))
	{
		str.assign("BINARY_FLOAT");
	}
	else if (ti == typeid(BinaryDoubleVar))
	{
		str.assign("BINARY_DOUBLE");
	}
	...
	else if (ti == typeid(CharVar))
	{
		char s_size[18];
		itoa(((CharVar&)var).GetStringBufSize(), s_size, 10);
		str.assign("CHAR(");
		str.append(s_size);
		str.append(")");
	}
