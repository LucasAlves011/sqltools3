...
static char THIS_FILE[] = __FILE__;
    ...
    bool CConnectDlg::m_OnLoginPlSqlBlockInitialized = false;
    std::wstring CConnectDlg::m_OnLoginPlSqlBlock;
    bool CConnectDlg::m_OnLoginPlSqlBlockEnabled = false;
	...
	m_profiles(m_adapter)
{
    ...
    m_data.EncryptProbe(m_masterPassword);
}
...
BEGIN_MESSAGE_MAP(CConnectDlg, CDialog)
    ...
    ON_COMMAND(ID_CON_COPY_ALL, OnCopyAll)
END_MESSAGE_MAP()
...
bool CConnectDlg::showDlg (CConnectSettingsDlg& dlg)
{
    ...
    dlg.m_OnLoginPlSqlBlockEnabled = m_data.m_OnLoginPlSqlBlockEnabled;
    
    m_data.DecryptOnLoginPlSqlBlock(m_masterPassword, dlg.m_OnLoginPlSqlBlock);

    if (dlg.DoModal() == IDOK)
    {
        // encript with the current password, DecryptAll/EncryptAll will be called later if the password is changed
        m_data.EncryptOnLoginPlSqlBlock(m_masterPassword, dlg.m_OnLoginPlSqlBlock); 
        m_data.m_OnLoginPlSqlBlockEnabled = dlg.m_OnLoginPlSqlBlockEnabled;
        ...
        m_OnLoginPlSqlBlock = dlg.m_OnLoginPlSqlBlock;
        m_OnLoginPlSqlBlockEnabled = dlg.m_OnLoginPlSqlBlockEnabled;
        m_OnLoginPlSqlBlockInitialized =  true;
...
//static 
bool CConnectDlg::GetOnLoginPlSqlBlock (string& block)
{
    if (!m_OnLoginPlSqlBlockInitialized)
    {
        ConnectDataXmlStreamer streamer(FILE_NAME);
        streamer.SetShallowRead(); 
        ...
        if (streamer.fileExists()) 
        {
            ...
            if (data.m_OnLoginPlSqlBlockEnabled)
                for (int attempt = 0; !data.TestPassword(m_masterPassword); ++attempt)
                {
                    if (attempt > 1)
                    {
                        AfxMessageBox(L"Invalid master password. On-login pl/sql block will be skipped.", MB_OK|MB_ICONEXCLAMATION);
                        break;
                    }

                    CPasswordDlg dlg(AfxGetMainWnd());
            ...
            m_OnLoginPlSqlBlockEnabled = data.m_OnLoginPlSqlBlockEnabled;
            if (m_OnLoginPlSqlBlockEnabled)
                data.DecryptOnLoginPlSqlBlock(m_masterPassword, m_OnLoginPlSqlBlock);
            m_OnLoginPlSqlBlockInitialized =  true;
        }
    }
    ...
    if (m_OnLoginPlSqlBlockEnabled)
    {
        block = Common::str(m_OnLoginPlSqlBlock);
        return !block.empty();
    }
            ...
            for (int attempt = 0; !m_data.TestPassword(m_masterPassword); ++attempt)
            {
                ...
                && MessageBox(L"Invalid master password. Try again?   ", MB_TITLE, MB_YESNO|MB_ICONSTOP) == IDNO)
                {
                    if (MessageBox(L"Would you like reset master password?   ", MB_TITLE, MB_YESNO|MB_ICONSTOP) == IDYES)
                    {
                        ...
                        m_data.EncryptOnLoginPlSqlBlock(m_masterPassword, m_OnLoginPlSqlBlock);
...
bool CConnectDlg::normalizeCurrent ()
{
    bool retVal = false;
    string user, password, alias, port, sid, mode, service;
    if (CCommandLineParser::GetConnectionInfo(Common::str(m_current.m_User), user, password, alias, port, sid, mode, service))
    {
        if (!password.empty() || !alias.empty())
        {
            if (port.empty())
            {
                m_current.m_Direct = false;
                m_current.m_User = Common::wstr(user);
                if (!password.empty()) 
                    m_current.m_DecryptedPassword = Common::wstr(password);
                if (!alias.empty())                                     // preserve existing alias when not specified
                    m_current.m_Alias = Common::wstr(alias);
                if (!mode.empty())                                      // sysdba/sysoper is valid for TNS alias too
                    m_current.m_Mode = ConnectionTasks::ToConnectionMode(mode);
            }
            else
            {
                m_current.m_Direct = true;
                m_current.m_User = Common::wstr(user);
                if (!password.empty()) 
                    m_current.m_DecryptedPassword = Common::wstr(password);
                m_current.m_Host = Common::wstr(alias);
                m_current.m_Port = Common::wstr(port);
                m_current.m_UseService = !service.empty();
                m_current.m_Sid = Common::wstr(!service.empty() ? service : sid);
                if (!mode.empty())
                    m_current.m_Mode = ConnectionTasks::ToConnectionMode(mode);
            }
            retVal = true;
...
try { EXCEPTION_FRAME;
    ...
    m_OnLoginPlSqlBlockEnabled = m_data.m_OnLoginPlSqlBlockEnabled;

    if (m_OnLoginPlSqlBlockEnabled)
        m_data.DecryptOnLoginPlSqlBlock(m_masterPassword, m_OnLoginPlSqlBlock);
    else
        m_OnLoginPlSqlBlock.clear();
    m_OnLoginPlSqlBlockInitialized = true;
...
try { EXCEPTION_FRAME;
    ...
    if (OpenClipboard() && EmptyClipboard())
    {
        ...
        if (m_current.m_Direct)
            str += L"@" + m_current.m_Host + L":" + m_current.m_Port 
                + (m_current.m_UseService ? L"/" : L":")
                + m_current.m_Sid;
        else
...
void CConnectDlg::OnCopyAll ()
try {
    EXCEPTION_FRAME;

    std::wstring str;

    if (int count = m_profiles.GetItemCount())
    {
        for (int item = 0; item < count; ++item)
        {
            int index = m_profiles.GetItemData(item);
            ConnectEntry conn;
            m_data.GetConnectEntry(index, conn, m_masterPassword);

            str += conn.m_User + L"/" + conn.m_DecryptedPassword;

            if (conn.m_Direct)
                str += L"@" + conn.m_Host + L":" + conn.m_Port
                + (conn.m_UseService ? L"/" : L":")
                + conn.m_Sid;
            else
                str += L"@" + conn.m_Alias;
        ...
        {
            CString msg;
            msg.Format(L"%d entries have been copied in Clipboard", count);
            AfxMessageBox(msg, MB_OK | MB_ICONINFORMATION);
        }
    }
    else
        AfxMessageBox(L"The list is empty — nothing to copy.", MB_OK | MB_ICONINFORMATION);
}
_DEFAULT_HANDLER_
