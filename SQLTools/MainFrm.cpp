...
#include <vector>
#include "AutomatedTesting/DispatchBridge.h"
...
static char THIS_FILE[] = __FILE__;
    ...
    static LPCWSTR lpszCodeOutline    = L"Code Outline";
...
BEGIN_MESSAGE_MAP(CMDIMainFrame, CWorkbookMDIFrame)
    ...
    ON_COMMAND(ID_SQL_OBJ_VIEWER_SETFOCUS, OnSqlObjViewerSetFocus)
    ON_UPDATE_COMMAND_UI(ID_SQL_OBJ_VIEWER_SETFOCUS, OnUpdate_SqlObjViewer)
    ...
    ON_COMMAND(ID_VIEW_CODE_OUTLINE, OnViewCodeOutline)
    ON_UPDATE_COMMAND_UI(ID_VIEW_CODE_OUTLINE, OnUpdate_ViewCodeOutline)
    //}}AFX_MSG_MAP
...
void CMDIMainFrame::EvOnConnect ()  // can be called twice on connect if on-login pl/sql block is defined
{
...
void CMDIMainFrame::EvOnSettingsChanged()
{
    m_wndDbSourceWnd.EvOnSettingsChanged();
}
...
const int IDC_MF_CODEOUTLINE_BAR = (AFX_IDW_CONTROLBAR_LAST - 16);
...
int CMDIMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    if (!m_wndCodeOutlineFrame.Create(lpszCodeOutline, this, CSize(200, 400), TRUE, IDC_MF_CODEOUTLINE_BAR)
    || !m_wndCodeOutline.Create(&m_wndCodeOutlineFrame, AFX_IDW_PANE_FIRST))
    {
        TRACE("Failed to create Code Outline\n");
        return -1;
    }
    HICON hCodeOutlineIcon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_CODE_OUTLINE), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), 0);
    m_wndCodeOutlineFrame.SetIcon(hCodeOutlineIcon, FALSE);
    m_wndCodeOutlineFrame.SetClient(&m_wndCodeOutline);
    m_wndCodeOutlineFrame.EnableDocking(CBRS_ALIGN_ANY);
    DockPane(&m_wndCodeOutlineFrame, AFX_IDW_DOCKBAR_LEFT);
    m_wndCodeOutlineFrame.ShowPane(FALSE, FALSE, TRUE); // hidden by default
...
void CMDIMainFrame::OnSqlObjViewerSetFocus()
{
    m_wndObjectViewerFrame.ShowPane(TRUE, FALSE, TRUE);

    CWnd* tree = m_wndObjectViewer.GetDlgItem(IDC_OIT_TREE);
    if (tree && tree->IsWindowEnabled())
        tree->SetFocus();
}
...
void CMDIMainFrame::OnViewCodeOutline()
{
    TogglePane(m_wndCodeOutlineFrame);
}

void CMDIMainFrame::OnUpdate_ViewCodeOutline(CCmdUI* pCmdUI)
{
    pCmdUI->SetCheck(m_wndCodeOutlineFrame.IsWindowVisible());
}
...
BOOL CMDIMainFrame::OnCopyData (CWnd*, COPYDATASTRUCT* pCopyDataStruct)
{
    ...
    if (pCopyDataStruct && pCopyDataStruct->dwData == STP_COPYDATA_REQ)
    {
        if (HandleProxyCopyData(pCopyDataStruct))
            return TRUE;
    }
    return FALSE;
}

BOOL CMDIMainFrame::HandleProxyCopyData(const COPYDATASTRUCT* pCDS)
{
    // Packet layout: [UINT32 cmd][UINT32 corrId][HWND reply][wchar_t payload[]]
    if (!pCDS || !pCDS->lpData || pCDS->cbData < sizeof(UINT32)*2 + sizeof(HWND))
        return FALSE;
    const BYTE* p = reinterpret_cast<const BYTE*>(pCDS->lpData);
    UINT32 cmd = *reinterpret_cast<const UINT32*>(p); p += sizeof(UINT32);
    UINT32 corr = *reinterpret_cast<const UINT32*>(p); p += sizeof(UINT32);
    HWND hReply = *reinterpret_cast<HWND const*>(p); p += sizeof(HWND);
    const wchar_t* payload = reinterpret_cast<const wchar_t*>(p);

    CString result;
    switch (cmd)
    {
    case STP_CMD_TEST_COLUMN_AC:
    case STP_CMD_TEST_SQL_FORMATTER:
        {
            // Forward to AutomatedTesting controller
            std::wstring res;
            AutomatedTesting_Dispatch(cmd, payload ? std::wstring(payload) : std::wstring(), res);
            result = res.c_str();
        }
        break;
    default:
    ...
    // Build response: [UINT32 corrId][wchar_t result[]]
    const size_t res_chars = result.GetLength() + 1;
    const size_t cb = sizeof(UINT32) + res_chars * sizeof(wchar_t);
    std::vector<BYTE> buffer(cb);
    BYTE* q = buffer.data();
    *reinterpret_cast<UINT32*>(q) = corr; q += sizeof(UINT32);
    memcpy(q, (LPCWSTR)result, res_chars * sizeof(wchar_t));

    COPYDATASTRUCT cdsResp{};
    cdsResp.dwData = STP_COPYDATA_RESP;
    cdsResp.cbData = static_cast<DWORD>(cb);
    cdsResp.lpData = buffer.data();
    if (IsWindow(hReply))
    {
        // Important: unblock the sender of the original WM_COPYDATA first to avoid deadlock.
        // Proxy thread was in SendMessage to us; we are about to SendMessage back to it.
        // ReplyMessage completes the original SendMessage on the sender side while we continue here.
        ::ReplyMessage(TRUE);
        auto retVal = ::SendMessage(hReply, WM_COPYDATA, reinterpret_cast<WPARAM>(m_hWnd), reinterpret_cast<LPARAM>(&cdsResp));
        retVal = retVal;
    }
    return TRUE;
}
...
void CMDIMainFrame::UpdateViewerAccelerationKeyLabels ()
{
    ...
    updateLabel(m_wndCodeOutlineFrame,  ID_VIEW_CODE_OUTLINE, lpszCodeOutline);
}
...
LRESULT CMDIMainFrame::OnSetMessageString (WPARAM wParam, LPARAM lParam)
{
    ...
    if (mru_file || mru_wks)
    {
        ...
        if (CWnd* pMessageBar = GetMessageBar())
        {
            ...
            shared_ptr<RecentFileList> list = theApp.GetRecentFileList();
...
void CMDIMainFrame::OnDestroyChild (CMDIChildWnd* pWnd)
{
    ...
    COEDocument* pDoc = dynamic_cast<COEDocument*>(pWnd->GetActiveDocument());
    m_wndCodeOutline.OnDocumentDestroyed(pDoc);
}
...
void CMDIMainFrame::OnActivateChild (CMDIChildWnd* pWnd)
{
    ...
    COEDocument* pDoc = pWnd ? dynamic_cast<COEDocument*>(pWnd->GetActiveDocument()) : nullptr;
    m_wndCodeOutline.SetDocument(pDoc);
}
