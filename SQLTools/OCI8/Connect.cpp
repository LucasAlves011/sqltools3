...
///////////////////////////////////////////////////////////////////////////////
    ...
    static HINSTANCE g_ociDll = NULL;
...
ConnectBase::ConnectBase (unsigned mode)
{
    ...
    if (!g_ociDll)
    {
        ...
        if (g_ociDll)
        {
            ...
            if (!version.empty())
            {
                if (!strncmp(version.c_str(), "21", sizeof("21") - 1))      m_clientVersoon = ecvClient21X;
                else if (!strncmp(version.c_str(),"19",  sizeof("19") - 1)) m_clientVersoon = ecvClient19X;
                else if (!strncmp(version.c_str(),"12",  sizeof("12")-1))   m_clientVersoon = ecvClient12X;
...
std::shared_ptr<BreakHandler> ConnectBase::CreateBreakHandler ()
{
    OCIError* errhp = 0;
    CHECK_ALLOC(OCIHandleAlloc(m_envhp, (dvoid**)&errhp, OCI_HTYPE_ERROR, 0, 0));
    return std::shared_ptr<BreakHandler>(new BreakHandler(m_svchp, errhp));
}
...
string ConnectBase::GetConnectIdentifier ()
{
    if (IsOpen())
    {
        char* dbname = nullptr;
        ...
        CHECK(OCIAttrGet((dvoid*)m_svchp, OCI_HTYPE_SVCCTX, &dbname, &dbname_len,  OCI_ATTR_INSTNAME, m_errhp));

        return string(dbname, dbname_len);
    }
    return string();
}
...
string Connect::GetConnectIdentifier ()
{
    if (GetClientVersion() <= OCI8::ecvClient11X)
    {
        if (IsOpen() && m_strConnectIdentifier.empty())
        {
            BuffCursor cursor(*this);
            cursor.Prepare("SELECT SYS_CONTEXT('USERENV', 'DB_NAME') FROM sys.dual");
            cursor.Execute();
            cursor.Fetch();
            cursor.GetString(0, m_strConnectIdentifier);
        }
        return m_strConnectIdentifier;
    }

    return ConnectBase::GetConnectIdentifier();
}
...
EServerVersion Connect::GetVersion ()
{
    if (m_version == esvServerUnknown)
    {
        ...
        if (!m_strVersion.empty())
        {
            if (!strncmp(m_strVersion.c_str(), "21.", sizeof("21.") - 1)) m_version = esvServer21X;
            else if (!strncmp(m_strVersion.c_str(), "19.", sizeof("19.") - 1)) m_version = esvServer19X;
            else if (!strncmp(m_strVersion.c_str(),"12.", sizeof("12.")-1)) m_version = esvServer12X;
...
void Connect::GetCurrentUserAndSchema (string& user, string& schema)
{
    BuffCursor cur(*this, 
        ...
        ? "SELECT USER, USER FROM sys.dual"
        : "SELECT SYS_CONTEXT('USERENV', 'CURRENT_USER'), SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') FROM sys.dual"
        );
