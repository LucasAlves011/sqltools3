...
bool AutoCursor::Fetch ()
{
    if (!m_endOfFetch)
    {
        vector<shared_ptr<Variable> >::const_iterator it = m_fields.begin();
        ...
        // allow variables to flush piecewise data filled by OCI after the
        // last callback (e.g. LongStringVar)
...
void AutoCursor::Define ()
{
    ...
    for (int col = 0; col < cols; col++) 
    {
		...
		case SQLT_CLOB:
            if (m_SkipLobs)
                fld = shared_ptr<Variable>(new DummyVar(true));
            else
                fld = shared_ptr<Variable>(new CLobVar(GetConnect(), m_StringLimit, (ECharForm)char_form));
			break;
		case SQLT_BLOB:
            if (m_SkipLobs)
                fld = shared_ptr<Variable>(new DummyVar(true));
            else
                fld = shared_ptr<Variable>(new BLobVar(GetConnect(), m_StringLimit, m_BlobHexRowLength/2));
			break;
		case SQLT_BFILE:
            if (m_SkipLobs)
                fld = shared_ptr<Variable>(new DummyVar(true));
            else
                fld = shared_ptr<Variable>(new BFileVar(GetConnect(), m_StringLimit, m_BlobHexRowLength/2));
			break;
        case SQLT_RID:
        case SQLT_RDD:
            fld = shared_ptr<Variable>(new StringVar(20+1));
			break;
        case SQLT_BIN:
            size *= 2;
        case SQLT_LBI:
            fld = shared_ptr<Variable>(new StringVar((size && size < m_StringLimit) ? size : m_StringLimit));
            break;
		case SQLT_CHR:
		case SQLT_LNG:
		case SQLT_AFC:
			...
			break;
        //case SQLT_LNG:
        //    fld = shared_ptr<Variable>(new LongStringVar(m_StringLimit));
        //    break;
        case SQLT_INT:
        case SQLT_FLT:
		case SQLT_NUM:
        ...
        case SQLT_IBDOUBLE:
            if (m_NumberFormat.empty())
                fld = shared_ptr<Variable>(new NumStrVar);
            else
                fld = shared_ptr<Variable>(new NumberVar(GetConnect(), m_NumberFormat));
			break;

		case SQLT_DAT:
        case SQLT_DATE:
            fld = shared_ptr<Variable>(new DateVar(GetConnect(), m_DateFormat));
            break;
        case SQLT_TIME:
            if (Connect::IsTimestampSupported())
                fld = shared_ptr<Variable>(new TimestampVar(GetConnect(), TimestampVar::TIME, m_TimeFormat));
		    break;
        case SQLT_TIMESTAMP:
            if (Connect::IsTimestampSupported())
                fld = shared_ptr<Variable>(new TimestampVar(GetConnect(), (TimestampVar::ESubtype)type, m_TimestampFormat));
			break;
        case SQLT_TIMESTAMP_TZ:
        case SQLT_TIMESTAMP_LTZ:
            if (Connect::IsTimestampSupported())
                fld = shared_ptr<Variable>(new TimestampVar(GetConnect(), (TimestampVar::ESubtype)type, m_TimestampTzFormat));
			break;
        case SQLT_INTERVAL_YM:
        case SQLT_INTERVAL_DS:
            if (Connect::IsIntervalSupported())
                fld = shared_ptr<Variable>(new IntervalVar(GetConnect(), (IntervalVar::ESubtype)type));
			break;

		case SQLT_NTY:
			{
				wchar_t* type_name   = nullptr;
				wchar_t* type_schema = nullptr;
				ub4 name_len = 0, schema_len = 0;

				CHECK(OCIAttrGet((dvoid*)paramd, OCI_DTYPE_PARAM, &type_name,   &name_len,   OCI_ATTR_TYPE_NAME,   GetOCIError()));
				CHECK(OCIAttrGet((dvoid*)paramd, OCI_DTYPE_PARAM, &type_schema, &schema_len, OCI_ATTR_SCHEMA_NAME, GetOCIError()));

				if (type_schema && type_name && schema_len > 0 && name_len > 0)
				{
					string str_schema = Common::str(type_schema, schema_len / sizeof(wchar_t));
					string str_name   = Common::str(type_name,   name_len   / sizeof(wchar_t));

					// Read the OCI typecode directly from the column parameter descriptor.
					// For any opaque type (XMLTYPE and similar) Oracle reports
					// OCI_TYPECODE_OPAQUE (58).  We intentionally ignore the return value
					// so that the attribute probe never throws; typecode stays 0 on older
					// clients that do not expose this attribute on a select-list descriptor.
					// The explicit SYS.XMLTYPE guard is kept as a reliable fallback.
					ub2 typecode = 0;
					OCIAttrGet((dvoid*)paramd, OCI_DTYPE_PARAM, &typecode, nullptr, OCI_ATTR_TYPECODE, GetOCIError());

					bool is_opaque = (typecode == OCI_TYPECODE_OPAQUE)
								  || (str_schema == "SYS" && str_name == "XMLTYPE");

					if (is_opaque)
					{
						// Opaque types cannot be serialised via NamedTypeVariable.
						fld = shared_ptr<Variable>(new DummyVar);
					}
					else
					{
						// Generic named type (object, VARRAY, nested table)
						fld = shared_ptr<Variable>(new NamedTypeVariable(GetConnect(), str_schema.c_str(), str_name.c_str(), m_StringLimit));
					}
				}
				break;
			}
