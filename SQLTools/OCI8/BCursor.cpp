...
bool BuffCursor::Fetch ()
{
    ...
    else if (!m_endOfFetch)
    {
        ...
        vector<shared_ptr<HostArray> >::iterator it = m_fields.begin();
...
void BuffCursor::Define ()
{
    ...
    for (int col = 0; col < cols; col++) 
    {
		...
		case SQLT_CLOB:
		case SQLT_AFC:
            fld = shared_ptr<HostArray>(new StringArray((size && size < m_strLimit) ? size : m_strLimit, m_prefetch));
			break;
        case SQLT_INT:
        case SQLT_FLT:
		case SQLT_NUM:
            fld = shared_ptr<HostArray>(new NumberArray(m_prefetch));
			break;
		case SQLT_DAT:
            fld = shared_ptr<HostArray>(new DateArray(m_prefetch, m_dateFormat));
			break;
		default:
...
void BuffCursor::Bind (const char* name, const char* value, int len)
{
    ...
    shared_ptr<Variable>& fld = m_boundFields[name] = shared_ptr<Variable>(new StringVar(value, len));
...
void BuffCursor::Bind (const char* name, int value)
{
    ...
    shared_ptr<Variable>& fld = m_boundFields[name] = shared_ptr<Variable>(new NumberVar(m_connect, value));
...
{
    vector<shared_ptr<HostArray> >::const_iterator it = m_fields.begin();
