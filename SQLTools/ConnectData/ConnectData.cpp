...
#include "COMMON\MyUtf.h"
    ...
    CMN_IMPL_PROPERTY(ConnectData, OnLoginPlSqlBlock,           "");
    CMN_IMPL_PROPERTY(ConnectData, OnLoginPlSqlBlockEnabled,    false);
...
bool ConnectData::DecryptOnLoginPlSqlBlock (const std::wstring& password, std::wstring& block)
{
    try
    {
        string buff;
        Decrypt(Common::str(password), m_OnLoginPlSqlBlock, buff);
        block = Common::wstr(buff);
        return true;
    }
    catch (const InvalidMasterPasswordException&) {}
...
void ConnectData::EncryptOnLoginPlSqlBlock (const std::wstring& password, const std::wstring& block)
{
    Encrypt(Common::str(password), Common::str(block), m_OnLoginPlSqlBlock);
}
...
void ConnectData::EncryptAll (const string& password)
{
    ...
    if (!m_decOnLoginPlSqlBlock.empty())
        Encrypt(password, m_decOnLoginPlSqlBlock, m_OnLoginPlSqlBlock);
    else
        m_OnLoginPlSqlBlock.clear();
...
void ConnectData::DecryptAll (const string& password)
{
    ...
    Decrypt(password, m_OnLoginPlSqlBlock, buff);
    m_decOnLoginPlSqlBlock = buff;
...
void ConnectData::EncryptProbe (const std::wstring& password)
{
    if (m_Probe == PROBE)
        Encrypt(Common::str(password), m_Probe.c_str(), m_Probe);
}
