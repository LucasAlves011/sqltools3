...
void CObjectViewerWnd::ShowObject (const std::string& name, const std::string& alias, const std::string& column)
{
...
void CObjectViewerWnd::setQuestionIcon ()
{
    ...
    struct BackgroundTask_FindObjectsForViewer : FindObjectsTask
    {
        ...
        BackgroundTask_FindObjectsForViewer (CObjectViewerWnd& viewerWnd, const string& input, const string& as, const string& col)
            ...
            else
            {
                if (m_result.size() > 0)
                {
                    ...
                    )
                    {
                        ...
                        m_viewerWnd.m_treeViewer.ShowObject(m_result.at(0), vector<string>(), alias);
                        if (!column.empty())
                        {
                            HTREEITEM hRoot = m_viewerWnd.m_treeViewer.GetRootItem();
                            m_viewerWnd.m_treeViewer.ScheduleSelectColumn(hRoot, column);
                        }
...
void CObjectViewerWnd::DoOK (const std::string& alias, const std::string& column)
{
    try {
        EXCEPTION_FRAME;
        ...
        if (UpdateData())
        {
            ...
            BkgdRequestQueue::Get().Push(TaskPtr(new BackgroundTask_FindObjectsForViewer(*this, Common::str(m_input), alias, column)));
        }
    }
