...
BEGIN_MESSAGE_MAP(CTreeViewer, CTreeCtrlEx)
    ...
    ON_WM_CHAR()
    ON_COMMAND(ID_SQL_OBJ_VIEWER_INSERT_AT_CUR, &CTreeViewer::OnInsertAtCursor)
END_MESSAGE_MAP()
...
string CTreeViewer::GetTooltipText (HTREEITEM hItem)
{
    ...
    */
    ...
    /*
    Selects a direct child of the root tree item whose name matches m_column (case-insensitive).
    Pushed after BackgroundTask_LoadTreeNodeOnExpand so the children are already inserted.
    */
    struct BackgroundTask_SelectColumn : Task
    {
        CTreeViewer& m_viewer;
        HTREEITEM    m_hRoot;
        string       m_column;

        BackgroundTask_SelectColumn (CTreeViewer& viewer, HTREEITEM hRoot, const string& column)
            : Task("Select column in tree", 0),
            m_viewer(viewer),
            m_hRoot(hRoot),
            m_column(column)
        {
        ...
        void ReturnInForeground ()
        {
            if (!(m_viewer.GetItemState(m_hRoot, TVIS_EXPANDED) & TVIS_EXPANDED))
                return;

            string upperTarget = m_column;
            for (auto& c : upperTarget) c = (char)::toupper((unsigned char)c);

            HTREEITEM hChild = m_viewer.GetChildItem(m_hRoot);
            while (hChild != NULL)
            {
                SafeTreeNodePtr node(m_viewer.m_pNodePool, m_viewer.GetItemData(hChild));
                if (node)
                {
                    string upperName = node->GetName();
                    for (auto& c : upperName) c = (char)::toupper((unsigned char)c);
                    if (upperName == upperTarget)
                    {
                        m_viewer.Select(hChild, TVGN_CARET);
                        m_viewer.EnsureVisible(hChild);
                        break;
                    }
                }
                hChild = m_viewer.GetNextSiblingItem(hChild);
...
void CTreeViewer::ShowObject (const ObjectDescriptor& objDesc, const std::vector<std::string>& drilldown, const std::string alias)
{
    m_requestCounter = 0;
    SetCursor(NULL);
    ...
    if (Object* object = m_pNodePool->CreateObject(objDesc))
    {
        ...
        if (!alias.empty())
        {
            if (object->GetType() == "TABLE" || object->GetType() == "VIEW")
            {
                if (TableBase* table = dynamic_cast<TableBase*>(object))
                    table->SetAlias(alias);
            }
            else if (object->GetType() == "SYNONYM")
                if (Synonym* synonym = dynamic_cast<Synonym*>(object))
...
void CTreeViewer::ScheduleSelectColumn (HTREEITEM hRoot, const std::string& column)
{
    if (hRoot && !column.empty())
        BkgdRequestQueue::Get().Push(TaskPtr(new BackgroundTask_SelectColumn(*this, hRoot, column)));
}
...
1) simple     - typically just a word (object name)
2) MULTI-LINE - multi-line text (e.g. procedure call with parameters)
3) COLUMN     - one entry per line

the table of drag & drop formats for a SINGLE selected object
---------------------------------------------------------------------------
| Object               | Default    | <Shift> pressed | <Ctrl> pressed   |
|-------------------------------------------------------------------------|
| Column               | name       | name + datatype | name             |
| Constraint           | name       | columns in line | columns in column|
| Container*           | name       | items in line   | items in column  |
| GrantedPrivelege     | label      | same            | same             |
| Index                | name       | columns in line | columns in column|
| Package              | name       | same            | same             |
| PackageBody          | name       | same            | same             |
| PackageProcedure/    | call with  | same            | call with params |
|   PackageFunction    | params     |                 | in column        |
| Partition/           | name       | partition clause| same as Shift    |
|   Subpartition       |            |                 |                  |
| Procedure            | call with  | same            | call with params |
|                      | params     |                 | in column        |
| Sequence             | name       | same            | same             |
| Synonym              | name       | same            | same             |
| TableBase            | name       | columns in line | columns in column|
| Trigger              | name       | same            | same             |
| Unknown              | name       | same            | same             |
---------------------------------------------------------------------------
...
If multiple objects are selected, each item is always formatted as "simple"
(short name only). Both <Shift> and <Ctrl> are ignored in this case.
*/

/*
* use table alias, let override table alias?
* use control as multiline switch for multiple columns formatted.
* show multi-column formatting in the hint
*/
...
const string CTreeViewer::GetSelectedItemsAsText (bool shift /*= false*/, bool ctrl /*= false*/)
{
    if (CObjectViewerWnd* parent = dynamic_cast<CObjectViewerWnd*>(GetParent()))
    {
        ...
        //bool aliasInLowercase = GetSQLToolsSettings().GetAutocompleteUseLowercaseForAliases();
        ...
        for (HTREEITEM hItem = GetFirstSelectedItem(); hItem != NULL; hItem = GetNextSelectedItem(hItem))
        {
            ...
            if (node)
            {
                if (const TableBase* tb = dynamic_cast<const TableBase*>(node->GetParent()))
                {
                    ctx.m_table_alias = tb->GetAlias();
                    //if (aliasInLowercase)
                    //    for (auto& c : ctx.m_table_alias) c = (char)::tolower((unsigned char)c);
                }
                else
                    ctx.m_table_alias.clear();
    ...
    if (!GetChildItem(hItem))
    {
        ...
        if (node)
        {
            ...
            if (node->IsComplete())
            {
                ...
                if (children && node->GetType() == "SYNONYM") // propagate table alias from synonym to table
                {
                    if (Synonym* synonym = dynamic_cast<Synonym*>(node.get()))
                        if (!synonym->GetAlias().empty())
                            if (TableBase* table = dynamic_cast<TableBase*>(node->GetChild(0)))
                                table->SetAlias(synonym->GetAlias());
                }
...
void CTreeViewer::BuildContexMenu (CMenu& menu, HTREEITEM hItem)
{
    ...
    SafeTreeNodePtr node(m_pNodePool, hItem ? GetItemData(hItem) : 0);
    Object* object = dynamic_cast<Object*>(node.get());
    bool canLoadDDL = (selectedCount == 1) && (object && object->IsTrueObject()) ? true : false;

    const wchar_t* cszINSERT_AT_CURSOR = GetSQLToolsSettings().GetObjectViewerCopyPasteOnDoubleClick()
        ? L"&Insert at Cursor\t<DblClick>; <Space>" : L"&Insert at Cursor\t<Space>";
    menu.AppendMenu(node ? MF_STRING : MF_STRING | MF_GRAYED, ID_SQL_OBJ_VIEWER_INSERT_AT_CUR, cszINSERT_AT_CURSOR);
    ...
    // have to reapply accel hint
    menu.ModifyMenu(ID_SQL_OBJ_VIEWER_INSERT_AT_CUR, node ? MF_BYCOMMAND : MF_BYCOMMAND | MF_GRAYED, ID_SQL_OBJ_VIEWER_INSERT_AT_CUR, cszINSERT_AT_CURSOR);
    if (node)
        menu.SetDefaultItem(ID_SQL_OBJ_VIEWER_INSERT_AT_CUR);
}
...
void CTreeViewer::OnSqlQuickCount()
{
    ...
    static 
    bool is_whitespace_line (const OpenEditor::OEStringW& str)
    {
    ...
    static 
    wchar_t first_non_whitespace_char (const OpenEditor::OEStringW& str)
    {
        for (int i = 0, len = str.length(); i < len; ++i)
            if (!iswspace(str.at(i)))
                return str.at(i);
    ...
    static 
    wchar_t last_non_whitespace_char (const OpenEditor::OEStringW& str)
    {
    ...
    // Scan backwards from (lineNo, inx) through the current line then up to 5
    // previous lines looking for the nearest alias.column pattern.  Returns the
    // alias part (e.g. "t" from "t.col"), or an empty string if none found.
    static
    std::string find_previous_alias (COEditorView* editor, int lineNo, int inx, const OpenEditor::OEStringW& currentLine)
    {
        for (int pass = 0; pass <= 5; ++pass)
        {
            OpenEditor::OEStringW buf;
            const OpenEditor::OEStringW* pLine;
            int scanTo;
            ...
            else
            {
                int l = lineNo - pass;
                if (l < 0) break;
                editor->GetLineW(l, buf);
                if (first_non_whitespace_char(buf) == L'/')
                    break;
                pLine = &buf;
                scanTo = buf.length();
            }
            ...
            for (int i = scanTo - 1; i >= 0; --i)
            {
                ...
                {
                    int j = i - 1;
                    while (j >= 0 && (iswalnum(pLine->at(j)) || pLine->at(j) == L'_'))
                        --j;
                    int start = j + 1;
                    if (start < i && (iswalpha(pLine->at(start)) || pLine->at(start) == L'_'))
                    {
                        std::wstring w;
    ...
    // Returns true when line (stripped of leading whitespace) matches
    // SELECT [alias.]column  with nothing else on the line.
    static
    bool matches_select_column (const OpenEditor::OEStringW& line)
    {
        int i = 0;
        int len = line.length();

        while (i < len && iswspace(line.at(i))) ++i;

        const wchar_t kw[] = L"SELECT";
        for (int k = 0; k < 6; ++k, ++i)
            if (i >= len || towupper(line.at(i)) != kw[k])
                return false;

        if (i >= len || !iswspace(line.at(i)))
            return false;
        while (i < len && iswspace(line.at(i))) ++i;

        // consume optional alias.
        int saved = i;
        while (i < len && (iswalnum(line.at(i)) || line.at(i) == L'_')) ++i;
        if (i < len && line.at(i) == L'.')
            ++i;
        else
            i = saved;

        // column name (at least one word char)
        if (i >= len || !(iswalnum(line.at(i)) || line.at(i) == L'_'))
            return false;
        while (i < len && (iswalnum(line.at(i)) || line.at(i) == L'_')) ++i;
...
/*
TODO: should i preserve (and show) selection order?
*/

void CTreeViewer::InsertSelected ()
{
...

    {
        ...
        CPLSWorksheetDoc* document = CPLSWorksheetDoc::GetActiveDoc();
        if (!document)
            return;

        COEditorView* editor = document->GetEditorView();

        // Determine whether the cursor is already right after "alias." so we
        // skip the alias prefix on the first item (it was already typed).
        bool cursorAfterDot = false;
        {
            Position curPos = editor->GetPosition();
            OEStringW curLine;
            editor->GetLineW(curPos.line, curLine);
            int curInx = editor->PosToInx(curPos.line, curPos.column);
            cursorAfterDot = (curInx > 0 && curInx <= (int)curLine.length() && curLine.at(curInx - 1) == L'.');
        }
        vector<Column*> selectedColumns;
        for (HTREEITEM hItem = GetFirstSelectedItem(); hItem != NULL; hItem = GetNextSelectedItem(hItem))
        {
            SafeTreeNodePtr node(m_pNodePool, GetItemData(hItem));
            if (Column* col = dynamic_cast<Column*>(node.get()))
                selectedColumns.push_back(col);
        }

        // Fall back to old logic if no columns are selected
        ...
        {
            SafeTreeNodePtr node(m_pNodePool, GetItemData(hItem));
            if (!dynamic_cast<Column*>(node.get()))
                SetItemState(hItem, 0, TVIS_SELECTED); // deselect non-column items from mixed selection
        }

        // Get alias from the root TableBase node
        string alias;
        {
            HTREEITEM hRoot = GetRootItem();
            if (hRoot)
            {
                SafeTreeNodePtr rootNode(m_pNodePool, GetItemData(hRoot));
                if (TableBase* tableBase = dynamic_cast<TableBase*>(rootNode.get()))
                    alias = tableBase->GetAlias();
            }
        }

        const auto s = GetSQLToolsSettings();
        const bool insertColumnsOnSeparateLines = s.GetAutocompleteInsertColumnsOnSeparateLines();
        const bool useLeadingComma              = s.GetAutocompleteUseLeadingComma();

        // Build per-column name strings (etfSHORT: name only, no type)
        vector<string> lines;
        for (Column* col : selectedColumns)
        {
            TextCtx ctx;
            ctx.m_form = etfSHORT;
            col->GetTextToCopy(ctx);
            lines.push_back(ctx.m_text);
        }

        string result;
        OpenEditor::Position insertPos;
        bool addCR = false;

        if (insertColumnsOnSeparateLines)
        {
            if (useLeadingComma)
            {
                // leading-comma style:
                ...
                for (const string& line : lines)
                {
                    ...
                    {
                        result += ", ";
                        if (!alias.empty()) { result += alias; result += '.'; }
                    }
                    else if (!cursorAfterDot && !alias.empty())
                    {
                        result += alias; result += '.';
                    }
                    result += line;
            ...
            else
            {
                ...
                //   col1,\n
                //   alias.col2,\n
                //   alias.col3,     <- trailing comma, cursor after it
                int inx = 0;
                int last_length = 0;
                for (const string& line : lines)
                {
                    ...
                    if (inx > 0)
                    {
                        if (!alias.empty()) { result += alias; result += '.'; last_length = (int)alias.size() + 1; }
                    }
                    else if (!cursorAfterDot && !alias.empty())
                    {
                        result += alias; result += '.'; last_length = (int)alias.size() + 1;
                    }
                    result += line;
                    last_length += (int)line.size();

                    if (inx < (int)lines.size() - 1)
                    { result += ",\n"; last_length += 2; }
                    else
                    { result += ",\n<CR>";  last_length += 1; addCR = true; }

                    inx++;
                }
                insertPos.line   = inx - 1;
                insertPos.column = last_length;
            }
        }
        else // single line
        {
            ...
            insertPos.line   = 0;
            insertPos.column = (int)result.size();
        }
        ...
        if (!result.empty())
        {
            if (OpenClipboard())
            {
                if (EmptyClipboard())
                {
                    Common::AppSetClipboardText(result.c_str(), result.length(), CF_TEXT);
                    Common::AppSetClipboardText("{code}", sizeof("{code}"), CF_PRIVATEFIRST);
                    if (addCR)
                        Common::AppSetClipboardText("{code+cr}", sizeof("{code+cr}"), CF_PRIVATEFIRST + 1);
                }
                CloseClipboard();
...
old_logic:
    {
        ...
        if (count > 0)
        {
            if (CPLSWorksheetDoc* document = CPLSWorksheetDoc::GetActiveDoc())
            {
                ...
                // Build single-line result from non-column items, space-separated
                string insertedText;
                for (HTREEITEM hItem = GetFirstSelectedItem(); hItem != NULL; hItem = GetNextSelectedItem(hItem))
                {
                    SafeTreeNodePtr node(m_pNodePool, GetItemData(hItem));
                    ...
                    {
                        TextCtx ctx;
                        ctx.m_form = count > 1 ? etfSHORT : etfDEFAULT;

                        // Single PackageProcedure/PackageFunction: get name only, then check
                        // if package name is already before cursor; prepend it if not.
                        if (count == 1)
                        {
                            if (PackageProcedure* pp = dynamic_cast<PackageProcedure*>(node.get()))
                            {
                                if (const Package* pkg = dynamic_cast<const Package*>(pp->GetParent()))
                                {
                                    // Get proc name without package prefix
                                    ctx.m_with_package_name = false;
                                    pp->GetTextToCopy(ctx);

                                    // Check if package name already precedes cursor
                                    ...
                                    // Check what precedes the cursor:
                                    //   "pkgname."  -> package and dot already there, insert proc name only
                                    //   "pkgname"   -> package already there but dot missing, insert ".procname"
                                    //   otherwise   -> insert "pkgname.procname"
                                    std::wstring wPkg = Common::wstr(pkgName);
                                    for (auto& c : wPkg) c = (wchar_t)::toupper((unsigned char)c);

                                    auto matchesPkg = [&](int endInx) -> bool {
                                        int nameStart = endInx - (int)pkgName.size();
                                        if (nameStart < 0) return false;
                                        std::wstring candidate(curLine.data() + nameStart, pkgName.size());
                                        for (auto& c : candidate) c = (wchar_t)::toupper((unsigned char)c);
                                        return candidate == wPkg;
                                    };

                                    bool pkgWithDot    = (curInx >= 1 && curLine.at(curInx - 1) == L'.')
                                                         && matchesPkg(curInx - 1);
                                    bool pkgWithoutDot = !pkgWithDot && matchesPkg(curInx);

                                    if (pkgWithDot)
                                        ; // proc name only — ctx.m_text already correct
                                    else if (pkgWithoutDot)
                                        ctx.m_text = '.' + ctx.m_text;   // prepend missing dot
                                    else
                                        ctx.m_text = pkg->GetName() + '.' + ctx.m_text;
                                }
                                else
                                {
                                    // No package parent accessible — fall back to default
                                    node->GetTextToCopy(ctx);
                                }

                                insertedText = ctx.m_text;
                                continue;
                            }
                        }
                ...
                // Add space padding if adjacent character is alphanumeric
                Position pos = editor->GetPosition();
                OEStringW line;
                editor->GetLineW(pos.line, line);
                int inx = editor->PosToInx(pos.line, pos.column);
                int len  = line.length();

                string prefix, suffix;
                wchar_t prevChr = (inx > 0)   ? line.at(inx - 1) : 0;
                wchar_t nextChr = (inx < len)  ? line.at(inx)     : 0;
                if (iswalnum(prevChr) && insertedText.at(0) != '.') prefix = ' ';
                if (iswalnum(nextChr)) suffix = ' ';

                string text = prefix + insertedText + suffix;
                if (OpenClipboard())
                {
                    if (EmptyClipboard())
                    {
...
void CTreeViewer::OnChar (UINT nChar, UINT nRepCnt, UINT nFlags)
{
    try {
        EXCEPTION_FRAME;
...
void CTreeViewer::OnInsertAtCursor ()
{
    try {
        EXCEPTION_FRAME;
