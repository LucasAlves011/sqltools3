...
#define ADD_FOLDER_TO_CHILDREN2(T, folder) { folder = m_treeNodePool.Create<T, Object>(this); \
    folder->SetSchemaNameType(GetSchema(), GetName(), GetType()); \
    m_children.push_back(folder); }
#define ADD_FOLDER_TO_CHILDREN3(obj, T) { T* folder = m_treeNodePool.Create<T, Object>(obj); \
    folder->SetSchemaNameType(obj->GetSchema(), obj->GetName(), obj->GetType()); \
    obj->m_children.push_back(folder); }
...
namespace ObjectTree {
    ...
    void prepare_cursor (OciCursor& cur, const char* sttm, Object& obj, bool skip_bind = false)
    {
        ...
        subst.AddPair("<VISIBILITY>", (ver > OCI8::esvServer10X) ? "visibility" : "NULL");
        subst << sttm;
...
void Synonym::Load (OciConnect& connect)
{
    ...
    if (GetRefDBLink().empty())
    {
        ...
        if (!m_Alias.empty())
            if (TableBase* table = dynamic_cast<TableBase*>(refObj))
                table->SetAlias(m_Alias);
...
void Table::Load (OciConnect& connect)
{
    ...
    vector<Property> properties, properties2;
    {
        OciCursor cur(connect);
        prepare_cursor(cur, 
            "SELECT <RULE> partitioned, temporary, duration, "
                "tablespace_name, last_analyzed, num_rows, iot_type "
                "FROM sys.all_tables "
            "WHERE owner = :schema AND table_name = :name", *this);

        const int cn_partitioned     = 0;
        const int cn_temporary       = 1;
        const int cn_duration        = 2;
        const int cn_tablespace_name = 3;
        const int cn_last_analyzed   = 4;
        const int cn_num_rows        = 5;
        const int cn_iot_type        = 6;

        cur.Execute();
        if (cur.Fetch())
        {
            string info = "[";
            if (OraMetaDict::Loader::IsYes(cur.ToString(cn_partitioned)))
            {
                info += "partitioned";
                m_Partitioned = true;
            }
            else if (OraMetaDict::Loader::IsYes(cur.ToString(cn_temporary)))
            {
                info += "temporary";
                properties.push_back(Property("Temporary", "YES"));
                properties.push_back(Property("Duration", cur.ToString(cn_duration)));
            }
            else 
            {
                properties.push_back(Property("Tablespace", cur.ToString(cn_tablespace_name)));
            }
            ...
            if (!cur.IsNull(cn_iot_type))
            {
                ...
                info += "iot";
                properties.push_back(Property("IOT Type", cur.ToString(cn_iot_type)));
            }

            properties2.push_back(Property("Number of rows", cur.ToString(cn_num_rows)));
            properties2.push_back(Property("Last analyzed", cur.ToString(cn_last_analyzed)));
    ...
    if (m_Partitioned)
    {
        OciCursor cur(connect);
        prepare_cursor(cur, 
            "SELECT <RULE> partitioning_type, subpartitioning_type, <INTERVAL>, "
            "partition_count, def_tablespace_name FROM sys.all_part_tables "
            "WHERE owner = :schema AND table_name = :name", *this);
        ...
        if (cur.Fetch())
        {
            ...
            string partition_count = cur.ToString(3);
            if (partition_count == "1048575") partition_count += " (limit)";
            properties.push_back(Property("Partition count", partition_count));
            properties.push_back(Property("Default tablespace", cur.ToString(4)));
        }
    }
    properties.insert(properties.end(), properties2.begin(), properties2.end());
...
void FkConstraint::Load (OciConnect& connect)
{
    ...
    const int cn_tab_inx_tablespace_name =  5;
    const int cn_tab_inx_partitioned     =  6;
    const int cn_tab_inx_visibility      =  7;
    const int cn_tab_inx_num_rows        =  8;
    const int cn_tab_inx_distinct_keys   =  9;
    const int cn_tab_inx_last_analyzed   = 10;

    const char* csz_tab_idx_sttm =
        "SELECT <RULE> owner, index_name, index_type, uniqueness, status, tablespace_name,"
                "partitioned, <VISIBILITY>, num_rows, distinct_keys, last_analyzed "
    ...
    struct IndexDesc
    {
        string owner, index_name, index_type, uniqueness, tablespace, status;
        string partitioned, visibility, num_rows, distinct_keys, last_analyzed;
        vector<string> column_list;
    };
...
void Indexes::Load (OciConnect& connect)
{
...

    {
        ...
        while (cur.Fetch())
        {
            ...
            index.tablespace    = cur.ToString(cn_tab_inx_tablespace_name);
            index.status        = cur.ToString(cn_tab_inx_status    );
            index.partitioned   = cur.ToString(cn_tab_inx_partitioned  );
            index.visibility    = cur.ToString(cn_tab_inx_visibility   );
            index.num_rows      = cur.ToString(cn_tab_inx_num_rows);
            index.distinct_keys = cur.ToString(cn_tab_inx_distinct_keys);
            index.last_analyzed = cur.ToString(cn_tab_inx_last_analyzed);
    ...
    for (; it != indexes.end(); ++it)
    {
        ...
        properties.push_back(Property("Index Type",    it->index_type));
        properties.push_back(Property("Partitioned",   it->partitioned));
        properties.push_back(Property("Visibility ",   it->visibility));
        
        if (OraMetaDict::Loader::IsYes(it->partitioned))
        {
            OciCursor cur(connect);
            prepare_cursor(cur, 
                "SELECT <RULE> partitioning_type, subpartitioning_type, <INTERVAL>, "
                "partition_count, def_tablespace_name FROM sys.all_part_indexes "
                "WHERE owner = :schema AND index_name = :name", *index);

            cur.Execute();
            if (cur.Fetch())
            {
                string partitioning_type = cur.ToString(0);
                properties.push_back(Property("Partitioning_type", partitioning_type));
                if (cur.ToString(1) != "NONE")
                {
                    string subpartitioning_type = cur.ToString(1);
                    properties.push_back(Property("Subpartitioning_type", subpartitioning_type));
                }
                if (!cur.IsNull(2))
                    properties.push_back(Property("Interval", cur.ToString(2)));
            ...
            properties.push_back(Property("Tablespace", it->tablespace));

        properties.push_back(Property("Number of rows",it->num_rows));
        properties.push_back(Property("Distinct keys", it->distinct_keys));
        properties.push_back(Property("Last Analyzed", it->last_analyzed));
        properties.push_back(Property("Status",        it->status));
        index->SetProperties(properties);

        if (OraMetaDict::Loader::IsYes(it->partitioned))
        {
            ADD_FOLDER_TO_CHILDREN3(index, IndexPartitions);
        }
...
//            param->SetPosition(position);
    ...
    {
        const char* cookie = "sys.all_procedures.subprogram_id";
        ...
        if (!con.GetSessionCookie(cookie, val))
        {
            try // some 10g releases might have this column
            {
                ...
                cursor.Prepare("SELECT subprogram_id FROM sys.all_procedures where 1=2");
                cursor.Execute();
    ...
    const char* csz_pack_proc_arg_sttm =
      "SELECT "
        "ap.procedure_name procedure_name,"
        "<SUBPROGRAM_ID> subprogram_id,"
        "aa.argument_name argument_name,"
        "aa.position position,"
        "aa.type_name type_name,"              // dont forger TYPE_OWNER
        "aa.type_subname type_subname,"
        "aa.data_type data_type,"
        "aa.overload overload,"
        "aa.in_out in_out,"
        "aa.default_value default_value "
        "FROM sys.all_procedures ap, sys.all_arguments aa "
        "WHERE ap.owner = :schema "
        "AND ap.object_name = :name "
        "<SUBPROGRAM_FILTER> "
        "AND ap.object_name = aa.package_name(+) "
        "AND ap.procedure_name = aa.object_name(+) "
        "AND ap.owner = aa.owner(+) "
        "AND aa.data_level(+) = 0 "
        "ORDER BY <SUBPROGRAM_ID>, aa.position";
...
void Package::Load (OciConnect& connect)
{
    ...
    Common::Substitutor subst;
    subst.AddPair("<RULE>", (ver < OCI8::esvServer10X) ? "/*+RULE*/" : "");
    subst.AddPair("<SUBPROGRAM_ID>", has_subprogram_id(connect) ? "ap.subprogram_id" : "aa.object_name||'.'||aa.overload");
    subst.AddPair("<SUBPROGRAM_FILTER>", has_subprogram_id(connect) ? "AND ap.subprogram_id > 0 AND Nvl(ap.overload, -1) = Nvl(aa.overload(+), -1) " : "");
...
void SubpartitionTemplate::Load (OciConnect& connect)
{
    ...
    const int cn_part_high_value         = 1;
    const int cn_part_subpartition_count = 2;
    const int cn_part_tablespace_name    = 3;
    const int cn_part_num_rows           = 4;
    const int cn_part_avg_row_len        = 5;
    const int cn_part_last_analyzed      = 6;
...
void Partitions::Load (OciConnect& connect)
{
    ...
    while (cur.Fetch())
    {
        ...
        partition->SetName(cur.ToString(cn_part_partition_name));
        partition->SetType("PARTITION");
        string high_value = cur.ToString(cn_part_high_value);
        if (!high_value.empty())
            partition->SetInfo("[" + high_value + "]");

        std::vector<Property> properties;
        properties.push_back(Property("Subpartitions",  cur.ToString(cn_part_subpartition_count)));
        properties.push_back(Property("Tablespace",     cur.ToString(cn_part_tablespace_name)));
        properties.push_back(Property("Number of rows", cur.ToString(cn_part_num_rows)));
        properties.push_back(Property("Avg row length", cur.ToString(cn_part_avg_row_len)));
        properties.push_back(Property("Last analyzed",  cur.ToString(cn_part_last_analyzed)));
...
void Partition::Load (OciConnect& connect)
{
    ...
    const int cn_inx_part_partition_name     = 0;
    const int cn_inx_part_high_value         = 1;
    const int cn_inx_part_subpartition_count = 2;
    const int cn_inx_part_tablespace_name    = 3;
    const int cn_inx_part_distinct_keys      = 4;
    const int cn_inx_part_num_rows           = 5;
    const int cn_inx_part_last_analyzed      = 6;
    const int cn_inx_part_status             = 7;

void IndexPartitions::Load (OciConnect& connect)
{
    bool subpartitioned = false;
    ...
    OciCursor cur(connect);
    prepare_cursor(cur, csz_ind_part_sttm, *this);

    cur.Execute();
    while (cur.Fetch())
    {
        IndexPartition* partition = m_treeNodePool.Create<IndexPartition, IndexPartitions>(this);
        partition->SetName(cur.ToString(cn_inx_part_partition_name));
        partition->SetType("PARTITION");
        string high_value = cur.ToString(cn_inx_part_high_value);
        if (!high_value.empty())
            partition->SetInfo("[" + high_value + "]");

        std::vector<Property> properties;
        properties.push_back(Property("High value", cur.ToString(cn_inx_part_high_value)));
        properties.push_back(Property("Subpartitions", cur.ToString(cn_inx_part_subpartition_count)));
        properties.push_back(Property("Tablespace", cur.ToString(cn_inx_part_tablespace_name)));
        properties.push_back(Property("Number of rows", cur.ToString(cn_inx_part_num_rows)));
        properties.push_back(Property("Distinct keys", cur.ToString(cn_inx_part_distinct_keys)));
        properties.push_back(Property("Last analyzed", cur.ToString(cn_inx_part_last_analyzed)));
        properties.push_back(Property("Status", cur.ToString(cn_inx_part_status)));
        partition->SetProperties(properties);           
...
void IndexPartition::Load (OciConnect& connect)
{
    if (Partitions* parent = dynamic_cast<Partitions*>(GetParent()))
    {
        OciCursor cur(connect);
        prepare_cursor(cur, csz_inx_subpart_sttm, *parent);
        cur.Bind(":partition_name", GetName().c_str());

        cur.Execute();
        while (cur.Fetch())
        {
            IndexPartition* partition = m_treeNodePool.Create<IndexPartition, IndexPartition>(this);
            partition->SetName(cur.ToString(0));
            partition->SetType("SUBPARTITION");
