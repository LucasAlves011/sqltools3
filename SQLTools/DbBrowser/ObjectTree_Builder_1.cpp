...
std::string IndexPartitions::GetLabel (TreeNode*) const
{
    return "Partitions";
}

std::string IndexPartition::GetLabel (TreeNode*) const
{
    return GetName() + " \t" + GetInfo();
}
...
std::string Column::GetTooltipText () const
{
    ...
    ctx.m_form = etfLINE;
    GetTextToCopy(ctx);
    add_row(text, "<i>Hint</i>", "<i>Drag with SHIFT to get : </i><b>" + ctx.m_text + "</b>");
...
std::string Index::GetTooltipText() const
{
    std::string text = Object::GetTooltipText();
...
std::string TableBase::GetTooltipText () const
{
    ...
    add_row(text, "<i>Alias</i>", "<i>" + GetAlias() + "</i>");
    close_table(text);
...
std::string Partition::GetTooltipText() const
{
    std::string text = Object::GetTooltipText();
...
std::string IndexPartition::GetTooltipText() const
{
    std::string text = Object::GetTooltipText();
    ...
    case etfCOLUMN:
        {
            ...
            for (unsigned int i = 0; i < count; ++i)
                if (const ProcedureParameter* param = dynamic_cast<const ProcedureParameter*>(GetChild(i)))
                    max_arg_length = max<unsigned int>(max_arg_length, param->GetName().size());
...
void IndexPartition::GetTextToCopy(TextCtx& ctx) const
{
    switch (ctx.m_form)
    {
    case etfDEFAULT:
