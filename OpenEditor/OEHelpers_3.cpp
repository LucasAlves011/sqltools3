...
int  Searcher::DoBatch (Storage* _storage, const wchar_t* text, ESearchBatch mode, Square& last)
{
    ...
    shared_ptr<Storage::UndoGroup> pUndoGroup;
    shared_ptr<Storage::NotificationDisabler> pDisabler;
    ...
    try
    {
        ...
        do
        {
            ...
            if (ctx.m_storage != storage2)
            {
                ...
                if (mode == esbReplace)
                    pUndoGroup = shared_ptr<Storage::UndoGroup>(new Storage::UndoGroup(*ctx.m_storage));

                if (mode != esbCount)
                    pDisabler  = shared_ptr<Storage::NotificationDisabler>(
