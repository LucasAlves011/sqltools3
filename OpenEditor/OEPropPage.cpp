...
COEPropPage::COEPropPage(const SettingsManager& manager, Settings& settings)  
...
{
    std::string name = m_manager.GetClassByPos(SendDlgItemMessage(IDC_OEC_LANGUAGE, CB_GETCURSEL))->GetName();
    m_manager.SetClassSettingsByLang(name, m_settings);
    doSelChangeLanguage();
}
