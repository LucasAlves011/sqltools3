...
{ 
    ...
    public:
        ...
        virtual void PutToken (const Token&, std::unique_ptr<SyntaxNode>&) = 0;
    ...
    public:
        ...
        // Outline extraction — storage is used to retrieve identifier text from token positions
        void GetOutlineItems (const Storage& storage, OutlineItems& out) const;
        bool HasAnyFailedNode () const;
        int GetTokenCount () const { return m_tokenCounter; }

    private:
        std::unique_ptr<ScriptNode> m_root;
        SyntaxNode* m_top;
        ...
        static void GetOutlineItemsImpl (const Storage& storage, const SyntaxNode* node, int depth, OutlineItems& out);
        static bool HasFailedNodeImpl   (const SyntaxNode* node);
    };
