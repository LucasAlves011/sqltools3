...
{
    m_nScanStartInx = 0;
    m_nScanEndInx   = (m_nScanCurrentLine < GetLineCount()) ? m_pStorage->GetLineLengthW(m_nScanCurrentLine) : 0; // raw index, NOT GetLineLength() which returns a screen position
...
bool EditContext::tabify (const wchar_t* str, int len, std::wstring& result, int startPos, int tabSpacing, bool leading)
{
    ...
    for (int i = 0; i < len && str[i]; )
    {
        if (str[i] == ' ')
        {
            ...
            int k = 0;
            for (; i + k + startPos < pos && str[i + k] == ' '; k++)
                ;
...
// Expands tab characters in str to the equivalent number of spaces, writing
// the result into result.
//
// Parameters:
//   str        - input wide-character string to process
//   len        - number of characters in str
//   result     - receives the untabified string; may alias str (in-place safe)
//   startPos   - logical column offset of str[0] used for tab-stop alignment
//   tabSpacing - number of columns between consecutive tab stops
//   leading    - when true, only expand leading tabs (stop at the first
//                non-tab character and copy the remainder verbatim)
//
