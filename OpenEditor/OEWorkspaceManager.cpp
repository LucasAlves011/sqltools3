...
void OEWorkspaceManager::Init (COEDocManager* pDocManager, COEMultiDocTemplate* pDocTemplate, 
    std::shared_ptr<RecentFileList>& recentFileList, std::shared_ptr<FavoritesList>&  favoritesList
)
{
    CString error;
    ...
    struct WorkspaceOpenAutosaved_Note : Note 
    {
        ...
        virtual void Deliver ()
        {
            ...
            if (AfxMessageBox((L"Terminated program snapshot \"" + name + L"\" found in Backup folder!\n\nWould you like to restore it?").c_str(), MB_ICONQUESTION|MB_YESNO) == IDYES)
            {
                ...
                OEWorkspaceManager::Get().DoOpen(file, path, true/*restore*/);
                file.Close();
...
void OEWorkspaceManager::DoSave (CFile& file, bool use_relative, const std::wstring& filepath)
{
    ...
    bool saveWorkspace = !m_workspacePath.empty();
    ...
    for (std::vector<COEDocument*>::iterator it = docs.begin(); it != docs.end(); ++it)
    {
        if (COEDocument* pOEDoc = *it)
        {
            ...
            if (saveWorkspace) // ugly way to save workspace path but the original workspace format does not have a commot section so saving into the first file attributes
            {
                fileEl->SetAttribute("workspace", Common::str(m_workspacePath));
                saveWorkspace = false;
            }
...
void OEWorkspaceManager::DoOpen (CFile& file, const std::wstring& filepath, bool restore /*= false*/)
{
    CWaitCursor wait;
    ...
    while (bytes && rec && !rec.is_eof())
    {
        ...
        if (const TiXmlElement* fileEl = doc.FirstChildElement("File"))
        {
            ...
            if (const char* workspace = fileEl->Attribute("workspace"))
            {
                try
                {
                    workspacePath = Common::wstr(workspace);
                }
                catch (std::range_error&)
                {
                    workspacePath = Common::wstr_from_ansi(workspace);
                }
            }
    ...
    if (restore && !workspacePath.empty())
    {
