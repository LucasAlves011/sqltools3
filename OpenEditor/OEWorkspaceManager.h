...
class OEWorkspaceManager : private boost::noncopyable
{
    ...
    std::shared_ptr<RecentFileList> m_pRecentFileList;
    std::shared_ptr<FavoritesList>  m_pFavoritesList;
...
public:
    ...
    void Init (COEDocManager* pDocManager, COEMultiDocTemplate* pDocTemplate, std::shared_ptr<RecentFileList>&, std::shared_ptr<FavoritesList>&);
    ...
    void DoOpen (CFile&, const std::wstring& path, bool restore = false);
