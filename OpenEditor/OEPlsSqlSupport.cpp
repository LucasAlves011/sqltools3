...
int PlSqlSupport::s_instanceCounter = 0;
    ...
    m_parser(m_analyzer)
{
    ...
    auto& settings =  m_pStorage->GetSettings();
    m_OutlineEnabled = settings.GetPlSqlOutlineEnabled();
    m_totalLineLimit = settings.GetPlSqlFileMaxLines()  ? settings.GetPlSqlFileMaxLines()  : INT_MAX;
    m_tokenLimit     = settings.GetPlSqlFileMaxTokens() ? settings.GetPlSqlFileMaxTokens() : INT_MAX;
}
...
bool PlSqlSupport::processText (int batchLimit)
{
    ...
    if (!m_done)
    {
        ...
        {
            if (m_currLine >= m_totalLineLimit
            || m_analyzer.GetTokenCount() >= m_tokenLimit) break;
            Common::OEStringW wbuff;
            m_pStorage->GetLineW(m_currLine, wbuff);
	        m_parser.PutLine(m_currLine, wbuff.data(), wbuff.length());
        }

        if (m_currLine >= nlines || m_currLine >= m_totalLineLimit
        || m_analyzer.GetTokenCount() >= m_tokenLimit)
        {
            m_done = true;
            ...
            {
                OutlineItems fresh;
                m_analyzer.GetOutlineItems(*m_pStorage, fresh);
                bool truncated     = (m_currLine >= m_totalLineLimit
                                   || m_analyzer.GetTokenCount() >= m_tokenLimit);
                bool hasSyntaxError = m_analyzer.HasAnyFailedNode();
                OutlineQuality quality = truncated      ? oqTruncated
                                       : hasSyntaxError ? oqSyntaxError
                                       : oqClean;
                if (quality == oqClean)
                {
                    // Parse completed cleanly — always update
                    m_lastOutline        = fresh;
                    m_lastOutlineQuality = oqClean;
                }
                else if (!fresh.empty() && m_lastOutline.empty())
                {
                    // Nothing committed yet — take what we have
                    m_lastOutline        = fresh;
                    m_lastOutlineQuality = quality;
                }
                else
                {
                    // Keep existing outline; only update quality flag for status line
                    m_lastOutlineQuality = quality;
                }
            }
        }
