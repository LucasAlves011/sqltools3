...
void ScriptFactory::RecognizeAndCreate (const RecognitionPattern& tokens, std::unique_ptr<SyntaxNode>& child)
{
    bool partial = false;
...
void ScriptFactory::Create (const RecognitionPattern& tokens, std::unique_ptr<SyntaxNode>& child)
{
    child.reset(new GenericSqlNode);
    feed(tokens, *child);
}
...
void DmlSqlFactory::RecognizeAndCreate (const Token& token, std::unique_ptr<SyntaxNode>& child)
{
	switch (token)
	{
		case etCASE:
...
void PlSqlFactory::RecognizeAndCreate (const Token& token, std::unique_ptr<SyntaxNode>& child)
{
