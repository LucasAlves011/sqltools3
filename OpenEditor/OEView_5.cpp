...
/*
    ...
    19/07/2005 R######, addded line numbers on printing
*/
...
//#define ADJUST_FONT_BY_WIDEST
    ...
    struct PrintContext
    {
        ...
        bool m_lineNumbers;
        int m_lineNumbersColumnWidth;
        shared_ptr<OpenEditor::HighlighterBase> m_highlighter;
...
//    const long logPixelsY     = pDC->GetDeviceCaps(LOGPIXELSY);
    ...
    pc.m_lineNumbers = GetSettings().GetPrintLineNumbers();
    ...
    if (pc.m_lineNumbers)
    {
        pc.m_lineNumbersColumnWidth  = CalculateDigitsInLatsLineNumber() /*+ 1*/;
        pc.m_Rulers[0].m_Indent     += pc.m_Rulers[0].m_CharSize * pc.m_lineNumbersColumnWidth;
        pc.m_Rulers[0].m_ClientSize -= pc.m_Rulers[0].m_CharSize * pc.m_lineNumbersColumnWidth;
        pc.m_Rulers[0].m_Count      -= pc.m_lineNumbersColumnWidth;
    }
...
void COEditorView::OnPrint (CDC* pDC, CPrintInfo* pInfo)
{
    try { EXCEPTION_FRAME;
        ...
        if (pInfo->m_bPreview)
        {
            ...
            CRect rect(pc.m_Rulers[0](-pc.m_lineNumbersColumnWidth), 
                    pc.m_Rulers[1](0),
        ...
        if (pc.m_lineNumbers)
        {
            ...
            ::FillRect(*pDC, &rect, (HBRUSH)::GetStockObject(LTGRAY_BRUSH));
        }
            ...
            pageLine++, pc.m_cacheLine++)
        {
            ...
            while (!tokenizer.Eol())
            {
                ...
                if (pos >= pc.m_cacheWordWrapOffset)
                {
                    ...
                    pos = pos - pc.m_cacheWordWrapOffset;
