...
namespace Common
{
    ...
    std::string getOsInfo ()
    {
        ...
        case VER_PLATFORM_WIN32_NT:
            ...
            {
			case 10:
                // Windows 11 is version 10.0 with build number >= 22000
                if ((osvi.dwBuildNumber & 0xFFFF) >= 22000) {
                    osInfo << ((osvi.wProductType != VER_NT_WORKSTATION) ? "Windows Server 2022" : "Windows 11");
                } else {
                    osInfo << ((osvi.wProductType != VER_NT_WORKSTATION) ? "Windows Server 2016" : "Windows 10");
                }
                break;
            case 6:
                switch (osvi.dwMinorVersion)
                {
				case 3: osInfo << ((osvi.wProductType != VER_NT_WORKSTATION) ? "Windows Server 2012 R2" : "Windows 8.1"); break;
