...
namespace Common
{
    ...
    {
    public :
        void *operator new (size_t, T* prt) throw() { return prt; }
    ...
    QuickArray<T>::QuickArray (size_t capacity,  size_t growBy)
    {
        FakeArray::castPtr(this)->construct(sizeof(T), capacity, growBy);
    }

    template <class T> inline
    QuickArray<T>::QuickArray (const QuickArray<T>& array)
    {
    ...
    template <class T> inline
    size_t QuickArray<T>::capacity () const
    {
        return m_capacity;
    }
    ...
    T& QuickArray<T>::at (size_t i)
    {
        ...
        //_ASSERTE(i < m_size && m_data);
        if (!(i < m_size && m_data))
            _RAISE(std::out_of_range("invalid QuickArray<T> subscript"));
    ...
    void QuickArray<T>::erase (size_t index, size_t count)
    {
        ...
        size_t i = index;
        for (size_t end = index + count; i < end && i < m_size; i++)
            operator[](i).~T();
