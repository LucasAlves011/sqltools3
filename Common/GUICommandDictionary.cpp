...
// 27.03.2005 bug fix, missing shortcut labels for double key accelerators
...
bool GUICommandDictionary::BuildAcceleratorTable (std::istream& is, HACCEL& accelTable)
{
    ...
    int i(0);
    for (; i < MAX_MENU_ITEMS_NUMBER && getline(is, buffer); /**/)
    {
        string token;
        VKey vk_key[2];
        Command command_id;
        ...
        istringstream istr(buffer.c_str(), buffer.size());
...
void GUICommandDictionary::AddAccelDescriptionToMenu (HMENU& hMenu, unsigned int extraSpaces)
{
    ...
    for (auto it = g_dblKeyAccels.begin(); it != g_dblKeyAccels.end(); ++it)
    {
        ...
        for (; it2 != it->m_keyToCmdMap.end(); it2++)
        {
            ...
            CmdToDescMap::iterator it3 = accelDesc.find(it2->second);

            if (it3 == accelDesc.end())
...
void GUICommandDictionary::GetDblKeyCommnds (int dblKeyAccelInx, std::vector<std::pair<Command,string> >&commands) 
{
    commands.clear();

    if (dblKeyAccelInx < static_cast<int>(g_dblKeyAccels.size()))
    {
        KeyToCmdMap::const_iterator it = g_dblKeyAccels[dblKeyAccelInx].m_keyToCmdMap.begin();
        KeyToCmdMap::const_iterator end = g_dblKeyAccels[dblKeyAccelInx].m_keyToCmdMap.end();
        for (;it != end; ++it)
        {
            CString text;
            text.LoadString(it->second);
            int pos = text.Find('\n');
            if (pos != -1) text.SetAt(pos, '\0');
