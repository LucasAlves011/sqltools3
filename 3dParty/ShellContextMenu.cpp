...
LRESULT CALLBACK CShellContextMenu::HookWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    return ::CallWindowProc (/*(WNDPROC) GetProp ( hWnd, TEXT ("OldWndProc"))*/OldWndProc, hWnd, message, wParam, lParam);
}

HRESULT CShellContextMenu::DoQueryContextMenu (LPCONTEXTMENU pContextMenu, HMENU hmenu, UINT indexMenu, UINT idCmdFirst, UINT idCmdLast, UINT uFlags)
{
...
UINT CShellContextMenu::ShowContextMenu(CWnd *pWnd, CPoint pt)
{
    ...
    DoQueryContextMenu (pContextMenu, m_Menu->m_hMenu, m_Menu->GetMenuItemCount (), MIN_ID, MAX_ID, CMF_NORMAL | CMF_EXPLORE);
    ...
    try
    {
        idCommand = m_Menu->TrackPopupMenu (TPM_RETURNCMD | TPM_LEFTALIGN, pt.x, pt.y, pWnd);
    }
    catch (...)
    {
...
void CShellContextMenu::SetObjects(CStringArray &strArray)
{
    ...
    free (olePath);
#else
    psfDesktop->ParseDisplayName (NULL, 0, (LPWSTR)(LPCWSTR)(strArray.GetAt (0)), NULL, &pidl, NULL);
#endif
    ...
    for (int i = 0; i < nItems; i++)
    {
        ...
        free (olePath);
#else
        psfDesktop->ParseDisplayName (NULL, 0, (LPWSTR)(LPCWSTR)strArray.GetAt (i), NULL, &pidl, NULL);
#endif
