...
HTREEITEM CTreePropSheet::CreatePageTreeItem(LPCTSTR lpszPath, HTREEITEM hParent /* = TVI_ROOT */)
{
	...
	if (!hItem)
	{
        ...
        // 14.04.2005 (ak) small customization, added m_bAllExpand
        if (m_bAllExpand)
            m_pwndPageTree->Expand(hParent, TVE_EXPAND);
...
BOOL CTreePropSheet::OnInitDialog() 
{
	...
	if (!m_bPageCaption)
	{
		...
		// MoveWindow(rect); 29.03.2005 bug fix, (ak) MoveWindow replaced with SetWindowPos
