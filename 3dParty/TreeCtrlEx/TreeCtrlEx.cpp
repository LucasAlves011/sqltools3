...
void CTreeCtrlEx::OnLButtonDown( UINT nFlags, CPoint point )
{
    ...
    || (!(nHitFlags & TVHT_ONITEMBUTTON) && hClickedItem && (GetStyle() & TVS_FULLROWSELECT)) // (ak) for fullrowselect 
    )
	{
		SetFocus();
...
void CTreeCtrlEx::OnKeyDown( UINT nChar, UINT nRepCnt, UINT nFlags ) 
{
	...
	else if ( nChar==VK_UP || nChar==VK_DOWN )
	{
		...
		if ( hNextItem )
		{
            ...
            SetItemState( hSelectedItem, 0, TVIS_SELECTED ); // (ak) deselect explicitly for fullrowselect mode 
...
HTREEITEM CTreeCtrlEx::GetPrevSelectedItem( HTREEITEM hItem )
{
    ...
    static struct WinVersion
    {
        WinVersion () : m_version(0)
        {
            ...
            m_version = osvi.dwMajorVersion;
        }

        operator int () { return m_version; }
...
///////////////////////////////////////////////////////////////////////////////
// workaround for behavior change of SelectItem in Win7 
...
BOOL CTreeCtrlEx::SelectItem (HTREEITEM hItem)
{
    if (winVersion > 5)
    {
        ...
        HTREEITEM hSelItem = GetFirstSelectedItem();
        for (; hSelItem != NULL; hSelItem = GetNextSelectedItem(hSelItem)) 
            selectedItems.push_back(hSelItem);
    
        BOOL retVal = CTreeCtrl::SelectItem(hItem);

        std::vector<HTREEITEM>::const_iterator it = selectedItems.begin();
        for (; it != selectedItems.end(); ++it)
	        SetItemState(*it, TVIS_SELECTED, TVIS_SELECTED );
