...
inline int max (int a, int b) { return std::max<int>(a, b); }
inline int min (int a, int b) { return std::min<int>(a, b); }
...
BOOL CPPHtmlDrawer::GotoURL(LPCTSTR url, int showcmd /* = SW_SHOW */)
{
    ...
    if ((INT_PTR)result <= HINSTANCE_ERROR) 
	{
        ...
        if (GetRegKey(HKEY_CLASSES_ROOT, _T(".htm"), key) == ERROR_SUCCESS) 
		{
            ...
            if (GetRegKey(HKEY_CLASSES_ROOT,key,key) == ERROR_SUCCESS) 
			{
                ...
                USES_CONVERSION;
                UINT result2 = WinExec(T2A(key),showcmd);
				return (result2 > HINSTANCE_ERROR) ? TRUE : FALSE;
            } //if
        } //if
    } //if
    return TRUE;
...
//		m_arrTableSizes.clear();
	...
	m_lfDefault.lfWeight = (BYTE)m_defStyle.nWeightFont;
						...
						case TAG_ICON:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								if (NULL != hIcon)
								{
									//ENG: Image with shadow or not?
									//RUS: Изображение с тенью или нет
									BOOL bShadow2 = IsImageWithShadow(si);
									...
									//ENG: If a shadow was enabled then set a real size
									//RUS: Если тень доступна, то устанавливаем реальный размер
									if (si.nWidth && si.nHeight && bShadow2)
									{
										sz.cx = si.nWidth + m_szOffsetShadow.cx;
										sz.cy = si.nHeight + m_szOffsetShadow.cy;
									} //if
...
CPPString CPPHtmlDrawer::GetParameterString(CPPString & str, int & nIndex, TCHAR chBeginParam /* = _T(':') */, CPPString strSeparators /* = _T(";") */)
{
	if (GetBeginParameter(str, nIndex, chBeginParam))
	{
		...
		if (0 != ch)
		{
			...
			{
				nIndex++;
				TCHAR ch2 = GetIndexNextChars(str, nIndex, _T("\""));
				if (_T('"') == ch2)
				{
					nIndex ++;
