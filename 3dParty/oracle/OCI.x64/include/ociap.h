...
/* 
   ...
   MODIFIED   (MM/DD/YY)
   apfwkr      04/29/26 - CI #39271263 of hashakta_proj-105036 from main
   rpingte     11/24/24 - CI #37317702 of rpingte_bug-37115068 from main
   rpingte     11/04/24 - Bug 37115068: Add OCIVectorTo/FromSparseArray
   taranand    01/11/24 - Proj 100812: Change OCIVectorToArray vdim description
   taranand    12/06/23 - Change vector type to format
   rpingte     10/27/23 - Proj 100812: add vdim argument in OCIVectorFromText
   rpingte     10/17/23 - Proj 100812: add vector support
   taranand    07/12/23 - Bug 35041402: Fix deprecated trace api
   rkanodia    06/25/23 - Bug 35513924: New parameter for OCIPipelineProcess
   johtsai     03/17/23 - Saga OCI External TM
                          and OCIMemoryFree2
   johtsai     12/03/22 - Proj 81334, bug 34943906: Saga OCI External TM
   vdikkala    12/23/21 - Proj 83228: OCI Pipeline Interface
                          Add OCIPipelineBegin(), OCIPipelineEnd(),
                          and OCIPipelineProcess()
   tbhosle     12/06/21 - Proj 87515: Table DDL Notification
   rkanodia    10/20/21 - Proj 82587: Add OCITraceEventSet/Reset functions
...
/*
   NAME
     OCISagaBegin - OCI SAGA BEGIN

   PARAMETERS
     svchp            (IN)  service context
     errhp            (IN)  error handle
     initiator_name   (IN)  name of initiator 
     coordinator_name (IN)  name of coordinator
     coordinator_type (IN)  type of coordinator to use  
     timeout          (IN)  timeout in seconds
     version          (IN)  version of sagas to use       
     saga_id          (OUT) id of new saga
     saga_id_len      (OUT) length of saga id
     mode             (IN)  mode

   DESCRIPTION
     Begin a new saga. coordinator_type can be either 
     OCI_SAGA_DEFAULT_COORDINATOR or OCI_SAGA_EXTERNAL_COORDINATOR.
   ...
   NOTES       
*/
sword OCISagaBegin(OCISvcCtx* svchp, OCIError* errhp, OraText* initiator_name,
                   OraText* coordinator_name, ub4 coordinate_type,
                   ub4 timeout, ub4 version, ub1* saga_id, sb2* saga_id_len, 
                   ub4 mode);

/*------------------------- OCISagaJoin ------------------------------*/
/*
   NAME
     OCISagaJoin - OCI SAGA JOIN

   PARAMETERS
     svchp            (IN)  service context
     errhp            (IN)  error handle
     saga_id          (IN)  id of new saga
     saga_id_len      (IN)  length of saga id
     participant_name (IN)  name of participant
     coordinator_name (IN)  name of coordinator     
     version          (IN)  version of sagas to use
     timeout          (IN)  timeout in seconds
     mode             (IN)  mode
   ...
   DESCRIPTION
     Join an existing saga. This is only allowed for external coordinators,
     as sendRequest will implicitly perform a join on a participant's
     behalf. A join is really a join (to attach context for escrow tracking)
     followed by a local begin.
   ...
   NOTES       
*/
sword OCISagaJoin(OCISvcCtx* svchp, OCIError* errhp, ub1* saga_id,
                  sb2* saga_id_len, OraText* participant_name, 
                  OraText *coordinator_name, ub4 version, ub4 timeout,
                  ub4 mode);

/*------------------------- OCISagaCommit------------------------------*/
/*
   NAME
     OCISagaCommit - OCI SAGA COMMIT
   ...
   PARAMETERS
     ...
     participant_name (IN)  name of participant
     saga_id          (IN)  id of new saga     
     force            (IN)  whether or not to force the commit
     mode             (IN)  mode
     
   DESCRIPTION
     Performs saga finalization (commit)
   ...
   NOTES       
*/
sword OCISagaCommit(OCISvcCtx* svchp, OCIError* errhp,
                    OraText* participant_name, ub1* saga_id, ub1 force,
                    ub4 mode);

/*------------------------- OCISagaRollback----------------------------*/
/*
   NAME
     OCISagaRollback - OCI SAGA ROLLBACK
   ...
   PARAMETERS
     ...
     participant_name (IN)  name of participant
     saga_id          (IN)  id of new saga     
     force            (IN)  whether or not to force the rollback
     mode             (IN)  mode
     
   DESCRIPTION
     Performs saga finalization (rollback)
   ...
   NOTES       
*/
sword OCISagaRollback(OCISvcCtx* svchp, OCIError* errhp,
                      OraText* participant_name, ub1* saga_id, ub1 force,
                      ub4 mode);
...
sword OCIMemoryAlloc2(void  *hdl, OCIError *err, void  **mem,
        OCIDuration dur, ub4 size, ub4 flags, const OraText *comment);

sword OCIMemoryResize(void  *hdl, OCIError *err, void  **mem,
                      ub4 newsize, ub4 flags);

sword OCIMemoryResize2(void  *hdl, OCIError *err, void  **mem,
                       ub4 newsize, ub4 flags, const OraText *comment);

sword OCIMemoryFree(void  *hdl, OCIError *err, void  *mem);

sword OCIMemoryFree2(void  *hdl, OCIError *err, void  *mem, const OraText *comment);
...
sword OCIDdlEventRegister(OCISvcCtx *svchp, OCIError *errhp, 
                          OCISubscription *subscrhp, ub1 type,
                          oratext **filter, ub4 filter_len,
                          ub4 include_operation, oratext *client_id, 
                          ub2 client_id_len);

sword OCIDdlEventUnregister(OCISvcCtx *svchp, OCIError *errhp, 
                            OCISubscription *subscrhp, ub1 type,
                            oratext **filter, ub4 filter_len);
...
sword OCIPipelineEnd(OCISvcCtx *svchp, ub4 timeout, OCIError *errhp, ub4 mode);


sword OCIPipelineProcess(OCISvcCtx             *svchp,
                         OCIPipelineOperationID pipeOpID,
                         ub4                    timeout,
                         OCIError              *errhp,
                         ub4                    mode);

/*------------------------End OCIPipeline Prototypes-------------------------*/


/* convenient apis /macros   - get third operation etc*/

/*-------------------------- OCITraceEventSet -------------------------------*/
/*
   NAME
     OCITraceEventSet - OCI trace event set

   PARAMETERS
     errhp         (IN)  error handle
     eventNum      (IN)  trace envent number
     level         (IN)  trace level 
     mode          (IN)  mode

   DESCRIPTION
     It will set trace event

   RETURNS
       OCI Status

   NOTES
       This can also be used to enable event tracing at run time.
*/

sword OCITraceEventSet(OCIError  *errhp,
                       ub8        eventNum,
                       ub4        level,
                       ub4        mode);
...
/*
   NAME
     OCITraceEventReset - OCI trace event reset
   ...
   DESCRIPTION
     It will disable trace event

   RETURNS
       OCI Status

   NOTES
       This can also be used to disable event tracing at run time.
*/

sword OCITraceEventReset(OCIError    *errhp,
                         ub8          eventNum,
                         ub4          mode);
...
/*
   NAME
     OCITraceWriteMessage - OCI trace write message
   ...
   DESCRIPTION
     It will write cutom message in trace file

   RETURNS
       OCI Status

   NOTES
       This can also be used to add user specific message in trace file.
       This function is to be used only for the public trace function
       OCITraceWriteMessage. It should not be used internally.

*/

sword OCITraceWriteMessage(OCIError   *errhp,
                           oratext    *message,
                           ub4         length);
...
/*
   NAME
     OCIVectorFromText - OCI Vector from text

   PARAMETERS
     vectord  (IN/OUT) - pointer to vector descriptor
     errhp    (IN/OUT) - error handle
     vformat  (IN)     - format of vector, refer to OCI_ATTR_VECTOR_FORMAT_*
     vdim     (IN)     - dimension of vector
     vtext    (IN)     - text representation of vector data
     vtextlen (IN)     - length of vtext data content
     mode     (IN)     - mode of operation

   DESCRIPTION
     This function converts a textual representation of vector to a OCIVector
     desciptor that can be used to bind the place holder for DMLs or used
     in descriptor attribute operations.

   RETURNS
     OCI_ERROR when an error is encountered, OCI_SUCCESS otherwise.
     OCI_INVALID_HANDLE when descriptor or error handle is invalid.

   NOTES
     The input text must contain comma separated vector values
     and should be enclosed in square brackets.
     Example vtext contents - "[20, 10, 1998]"
*/
sword OCIVectorFromText(OCIVector *vectord, OCIError *errhp, ub1 vformat,
                        ub4 vdim, const OraText *vtext, ub4 vtextlen,
                        ub4 mode);

/*------------------------- OCIVectorToText -----------------------------*/
/*
   NAME
     OCIVectorToText - OCI Vector to text

   PARAMETERS
     vectord  (IN)     - pointer to vector descriptor
     errhp    (IN)     - error handle
     vtext    (IN/OUT) - pointer to memory where textual representation will 
                         be returned
     vtextlen (IN/OUT) - pointer to length of vtext memory
     mode     (IN)     - mode of operation

   DESCRIPTION
     This function converts vector data in OCIVector descriptor to textual
     representation that can be used to display the content or used
     in OCIVectorFromText to construct another OCIVector descriptor.
   ...
   NOTES
     The vector is converted to text consisting of comma seperated values
     and enclosed in square brackets.
     Example vtext output contents  - "[20, 10, 1998]"
*/
sword OCIVectorToText(OCIVector *vectord, OCIError *errhp, OraText *vtext,
                      ub4 *vtextlen, ub4 mode);

/*------------------------- OCIVectorFromArray -----------------------------*/
/*
   NAME
     OCIVectorFromArray - OCI Vector from array
   ...
   PARAMETERS
     ...
     vformat  (IN)     - format of vector, refer to OCI_ATTR_VECTOR_FORMAT_*
     vdim     (IN)     - dimension of vector
     vecarray (IN)     - pointer to an array of elements of vformat
     mode     (IN)     - mode of operation

   DESCRIPTION
     This function converts an array representation of elements to a OCIVector
     desciptor that can be used to bind the place holder for DMLs or used
     in descriptor attribute operations.
...
/*
   NAME
     OCIVectorToArray - OCI Vector to array

   PARAMETERS
     vectord  (IN)     - pointer to vector descriptor
     errhp    (IN)     - error handle
     vformat  (IN)     - format of vector, refer to OCI_ATTR_VECTOR_FORMAT_*
     vdim     (IN/OUT) - pointer to number of dimensions in array buffer,
                         output value is actual number of dimensions in the
                         vector
     vecarray (OUT)    - pointer to an array of elements of vformat
     mode     (IN)     - mode of operation

   DESCRIPTION
     This function converts contents of OCIVEctor descriptor to an array
     representation of elements that can be used to bind the place holder for
     DMLs or used in descriptor attribute operations.
...
/*
   NAME
     OCIVectorFromSparseArray - OCI Vector from SPARSE Array

   PARAMETERS
     vectord  (IN/OUT) - pointer to vector descriptor
     errhp    (IN/OUT) - error handle
     vformat  (IN)     - format of vector one of:
                         (OCI_ATTR_VECTOR_FORMAT_INT8,
                          OCI_ATTR_VECTOR_FORMAT_FLOAT32,
                          OCI_ATTR_VECTOR_FORMAT_FLOAT64,
                          OCI_ATTR_VECTOR_FORMAT_BINARY)
     vdim     (IN)     - total number of dimensions in SPARSE vector
     indices  (IN)     - number of dimensions in index array
     indarray (IN)     - pointer to an array containing non zero dimension
                         index position values
     vecarray (IN)     - pointer to an array containing array of elements of
                         vformat values in SPARSE vector
     mode     (IN)     - mode of operation

   DESCRIPTION
     This function converts a SPARSE array representation of elements to a
     OCIVector desciptor that can be used to bind the place holder for DMLs
     or used in descriptor attribute operations.
   ...
   NOTES
     The indarray input will contain the indices of non zero dimensions.
     The vecarray input will contain array of elements of vformat of those
     indices.
     For eg.
     The following dense array - [0, 1.0, 0, 1.0, 0] will have this input-
     vdim = 5, indices = 2, indarray = [2,4], vecarray = [1.0, 1.0]
*/

sword OCIVectorFromSparseArray(OCIVector *vectord, OCIError *errhp,
                               ub1 vformat, ub4 vdim, ub4 indices,
                               void *indarray, void *vecarray, ub4 mode);

/*------------------------- OCIVectorToSparseArray -----------------------*/
/*
   NAME
     OCIVectorToSparseArray - OCI Vector to SPARSE Array

   PARAMETERS
     vectord  (IN/OUT) - pointer to vector descriptor
     errhp    (IN/OUT) - error handle
     vformat  (IN)     - format of vector, refer to OCI_ATTR_VECTOR_FORMAT_*
     vdim     (IN/OUT) - pointer containing size of indarray. Output value
                         contains total number of dimensions in SPARSE vector
     indices  (OUT)    - pointer that will be populated with number of
                         dimensions in indarray
     indarray (OUT)    - pointer to buffer that will be populated with indices
                         of non zero SPARSE vector dimensions
     vecarray (OUT)    - pointer to buffer that will be populated with an array
                         of elements of vformat in SPARSE vector
     mode     (IN)     - mode of operation

   DESCRIPTION
     This function converts contents of OCIVector descriptor to a SPARSE array
     representation of elements that can be used to bind the place holder for
     DMLs or used in descriptor attribute operations.
   ...
   NOTES
     The indarray output will contain the indices of non zero dimensions.
     The vecarray output will contain the array of elements of vformat of those
     indices.
     For eg.
     The following dense array - [0, 1.0, 0, 1.0, 0] will have this output-
     vdim = 5, indices = 2, indarray = [2,4], vecarray = [1.0, 1.0]
*/

sword OCIVectorToSparseArray(OCIVector *vectord, OCIError *errhp, ub1 vformat,
                             ub4 *vdim, ub4 *indices, void *indarray,
                             void *vecarray, ub4 mode);

/*------------------------- OCIEnquoteLiteral -----------------------*/
/*
  NAME
    OCIEnquoteLiteral - OCI Enquote Literal

  PARAMETERS
    envhp       (IN)     - evironment handle
    errhp       (IN/OUT) - error handle
    in_str      (IN)     - pointer to input string
    in_str_len  (IN)     - input string length
    out_str     (IN/OUT) - pointer to output string
    out_str_len (IN)     - output string length
    mode        (IN)     - mode of operation

  DESCRIPTION
    This function is an OCI C-implementation of DBMS_ASSERT.ENQUOTE_LITERAL,
    which encloses the input string in single quotes. No additional quotes
    are added if the input is already in quotes. The quoted string is then
    checked to see if all single quotes except leading and trailing 
    characters paired with adjacent single quotes.

  RETURNS
    The actual size of the result string (including null-terminator at the
    end) if the verification succeeds.
    OCI_ERROR if the verification fails with actual error code in the errhp

  NOTES
    As the output string will be null-terminated, the caller must make sure
    that enough space is allocated for out_str; e.g.,, (size of out_str) >=
    (size of in_str + 2 (quotes) + 1 (null-terminator)) unless the input is
    already quoted and null-terminated. In case the output buffer is not
    sufficient, ORA-22620 will be raised.
    The input string does not need to be null-terminated. 
    This function does not trim leading/trailing whitespace characters.
*/

sword OCIEnquoteLiteral(OCIEnv *envhp, OCIError *errhp, oratext *in_str, 
                        ub4 in_str_len, oratext *out_str, ub4 *out_str_len, 
                        ub4 mode);

/*------------------------- OCIEnquoteName -----------------------*/
/*
  NAME
    OCIEnquoteName - OCI Enquote Name
  ...
  PARAMETERS
    ...
    out_str     (IN/OUT) - pointer to output string
    out_str_len (IN)     - output string length
    capitalize  (IN)     - TRUE if output string should be converted 
                           to upper-case
    mode        (IN)     - mode of operation

  DESCRIPTION
    This function is an OCI C-implementation of DBMS_ASSERT.ENQUOTE_NAME,
    which encloses the input string in double quotes. No additional quotes
    are added if the input is already in quotes. The quoted string is then
    checked to see if it is a valid (quoted) simple SQL name.
...
sword OCIEnquoteName(OCIEnv *envhp, OCIError *errhp, oratext *in_str,
                     ub4 in_str_len, oratext *out_str,ub4 *out_str_len, 
                     boolean capitalize, ub4 mode);

/*------------------------- OCIQualifiedSqlNameCheck -----------------------*/
/*
  NAME
    OCIQualifiedSqlNameCheck - OCI Qualified SQL Name Check
  ...
  DESCRIPTION
    This function is an OCI C-implementation of DBMS_ASSERT.QUALIFIED_SQL_NAME,
    which verifies that the input string is a qualified SQL name. A qualified
    SQL name is expressed by the following grammar.
  ...
  RETURNS
    OCI_SUCCESS if the input string is a qualified SQL name.
    OCI_ERROR if the input string is NOT a qualified SQL name.

  NOTES
    The input string does not need to be null-terminated.
    Like DBMS_ASSERT.QUALIFIED_SQL_NAME, "" (i.e., an empty string) is
    considered a valid qualified SQL name.
*/

sword OCIQualifiedSqlNameCheck(OCIEnv *envhp, OCIError *errhp, 
                            oratext *in_str, ub4 in_str_len, ub4 mode);  
                          
/*------------------------- OCISimpleSqlNameCheck -----------------------*/
/*
  NAME
    OCISimpleSqlNameCheck - OCI Simple SQL Name Check
  ...
  DESCRIPTION
    This function is an OCI C-implementation of DBMS_ASSERT.SIMPLE_SQL_NAME,
    which verifies that the input string is a simple SQL name. A simple SQL
    name must meet the following conditions.
      - The name must begin with an alphabetic character. It may contain
        alphanumeric characters as well as the characters _, $, and # in the
        second and subsequent character positions.
      ...
      - The input parameter may have any number of leading and/or trailing
        white space characters.
      - The length of the name is not checked.

  RETURNS
    OCI_SUCCESS if the input string is a simple SQL name.
    OCI_ERROR if the input string is NOT a simple SQL name.

  NOTES
    The input string does not need to be null-terminated.
*/
sword OCISimpleSqlNameCheck(OCIEnv *envhp, OCIError *errhp, 
                            oratext *in_str, ub4 in_str_len, ub4 mode);
