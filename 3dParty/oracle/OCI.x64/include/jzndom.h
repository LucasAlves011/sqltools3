...
/*
   ...
   MODIFIED   (MM/DD/YY)
   apfwkr      05/18/26 - CI #39398372 of zliu_bug-38636001 from main
   ...
   dmcmahon    06/06/26 - Bug 39499715: JsonDomSetFieldNative()
   dmcmahon    04/20/26 - Bug 39233764: nlslen_jznScalarVal for NLSSORT
   saguerra    03/26/26 - Bug 39131948: NLSSORT related flags
...
#define nlslen_jznScalarVal            uval_jznScalarVal.val.xlen
...
sword   jznScalarCompare(jznScalarVal *val1, jznScalarVal *val2, uword flags);
...
/*
** Note: bits 0x00FFFF00 reserved for JZNANY_FLAG values
*/

/* If the LHS or RHS of a comparison is already an NLSSORT image */
#define JZNDOM_COMPARE_FLAG_NLS_LHS    0x01000000
#define JZNDOM_COMPARE_FLAG_NLS_RHS    0x02000000
#define JZNDOM_COMPARE_FLAG_NLS_SAVE   0x08000000
 ...
 */
#define JZNANY_FLAG_NLSSORT        0x40000  /* NLSSORT conversion on strings */
#define JZNANY_FLAG_CONCATENATED   0x80000  /* Building concatenated image   */
...
/*
** Set or replace a field in a DOM object given a name string in the
** working character set of the associated XmlCtx. <node> is an IN/OUT
** argument. On input, *node contains the proposed new field item.
** On output, *node returns the old field item (if any) as an orphan node,
** or a NULL if the field didn't already exist in the object.
** If the input *node is NULL, this is equivalent to just unlinking
** (but not deleting) the field, leaving the object without the given
** field. Returns JZNERR_OK on success, otherwise an error code.
** If a conversion is performed, it will reuse an internal conversion
** associated with the document.
*/
sword jznDomSetFieldNative(JsonDomDoc *doc, JsonDomObject *obj,
                           oratext *name, ub2 len, JsonDomNode **node);
