...
void CPropTreeInfo::OnPaint() 
{
	...
	// draw label
	dc.DrawText(txt, &ir, DT_SINGLELINE|DT_CALCRECT|DT_NOPREFIX);
	dc.DrawText(txt, &ir, DT_SINGLELINE|DT_NOPREFIX);
	...
	if (pItem)
        ...
        txt = "Select an item to see its description"; // (ak)

	dc.SelectObject(m_pProp->GetNormalFont());
	dc.DrawText(txt, &ir, DT_WORDBREAK|DT_NOPREFIX);
}
