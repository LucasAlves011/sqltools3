...
void CDirTreeCtrl::DisplayPath (HTREEITEM hParent, LPCTSTR strPath, BOOL skipFolders /*= FALSE*/)
{
    ...
    if ( m_bFiles )
    {
        ...
        else
        {
            ...
            {
                CString strPathFiles2 = strPath;
                if ( strPathFiles2.Right(1) != _T("\\") )
                    strPathFiles2 += _T("\\");
                strPathFiles2 += m_strFilter.GetAt(i);

                BOOL bFind2 = find.FindFile(strPathFiles2);
                while ( bFind2 )
                {
                    bFind2 = find.FindNextFile();
                    if (!find.IsDirectory())
